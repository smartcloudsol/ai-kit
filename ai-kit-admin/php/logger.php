...
class Logger
{
     ...
     * @var \SmartCloud\WPSuite\AiKit\Admin|null
     */
    private static $admin = null;

    /**
     * Get or create Admin instance (singleton pattern)
     * @return \SmartCloud\WPSuite\AiKit\Admin|null
     */
    private static function getAdmin()
    {
        if (self::$admin === null && class_exists('\SmartCloud\WPSuite\AiKit\Admin')) {
            self::$admin = new \SmartCloud\WPSuite\AiKit\Admin();
        }
        return self::$admin;
    }
    ...
    public static function log(string $message, string $level = self::INFO, array $context = []): void
    {
        ...
        if ($admin && method_exists($admin, 'getSettings')) {
            ...
            // getSettings() returns an AiKitSettings object

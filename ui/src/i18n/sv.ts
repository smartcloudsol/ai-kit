export const svDict: Record<string, string> = {
  "{name} is thinking…": "{name} funderar…",
  "Sending request…": "Skickar begäran…",
  "Waiting for reply…": "Väntar på svar…",
  "Reconnecting…": "Återansluter…",
  "Checking sources…": "Kontrollerar källor…",
  "Knowledge base searched": "Kunskapsbasen har sökts igenom",
  "Web search completed": "Webbsökning slutförd",
  "Calculation completed": "Beräkning slutförd",
  "Script completed": "Skript slutfört",
  "Check completed": "Kontroll slutförd",
  "Action completed": "Åtgärd slutförd",
  "Action failed": "Åtgärden misslyckades",
  "Search query: {query}": "Sökfråga: {query}",
  "Categories: {count}": "Kategorier: {count}",
  "Subcategories: {count}": "Underkategorier: {count}",
  "Tags: {count}": "Taggar: {count}",
  "Results: {count}": "Resultat: {count}",
  "Retry": "Försök igen",
  "The answer could not be verified against its cited sources. Please try again.": "Svaret kunde inte verifieras mot de angivna källorna. Försök igen.",
  "The knowledge base search could not be completed. Please try again.": "Sökningen i kunskapsbasen kunde inte slutföras. Försök igen.",
  "The answer's source check could not be completed. Please try again.": "Kontrollen av svarets källor kunde inte slutföras. Försök igen.",
  "Action interrupted": "Åtgärd avbruten",
  "Failed actions: {count}": "Misslyckade åtgärder: {count}",
  "--- Select ---": "--- Välj ---",
  "A response is still running. Wait for it to finish or cancel it.": "Ett svar genereras fortfarande. Vänta tills det är klart eller avbryt det.",
  Accept: "Acceptera",
  "Accept response": "Godkänn svar",
  Add: "Lägg till",
  "Add image": "Lägg till bild",
  "Additional instructions to guide the AI.":
    "Ytterligare instruktioner för att vägleda AI:n.",
  "AI Assistant": "AI-assistent",
  "AI Summary": "AI-sammanfattning",
  "Alt Text": "Alternativtext",
  "An error occurred.": "Ett fel inträffade.",
  "An unknown error occurred.": "Ett okänt fel uppstod.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Svar från {name} genereras av AI och kan innehålla fel. Kontrollera viktig information.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Jag hittade inte tillräckligt med tillförlitlig information för att svara. Formulera om frågan eller avgränsa ämnet.",
  "The AI response took too long. Please try again.":
    "AI-svaret tog för lång tid. Försök igen.",
  Arabic: "Arabiska",
  "Are you sure you want to reset the conversation?":
    "Är du säker på att du vill återställa konversationen?",
  "As-Is": "Oförändrat",
  "as-is": "som det är",
  "Ask anything…": "Fråga vad som helst…",
  "Ask me": "Fråga mig",
  Assistant: "Assistent",
  "Assistant is thinking…": "Assistenten funderar…",
  "Audio message": "Ljudmeddelande",
  "Audio no longer available": "Ljud är inte längre tillgängligt",
  "Audio recorded": "Ljud inspelat",
  auto: "Automatiskt",
  "Auto-detect": "Automatisk identifiering",
  "Calculating…": "Beräknar…",
  Cancel: "Avboka",
  Caption: "Rubrik",
  Casual: "Informell",
  casual: "tillfällig",
  Categories: "Kategorier",
  "Checking…": "Kontrollerar…",
  "Checking capabilities…": "Kontrollerar kapacitet…",
  "Checking capabilities...": "Kontrollerar kapacitet…",
  Chinese: "kinesiska",
  Clear: "Rensa",
  "Clear audio": "Rensa ljud",
  "Click again to confirm": "Klicka igen för att bekräfta",
  Close: "Stäng",
  "Close chat": "Stäng chatten",
  Confirm: "Bekräfta",
  Corrected: "Rättad",
  "Creating final summary...": "Skapar slutlig sammanfattning...",
  "Creating LanguageDetector session": "Skapar LanguageDetector-session",
  "Creating LanguageModel session": "Skapar LanguageModel-session",
  "Creating Proofreader session": "Skapa korrekturläsarsession",
  "Creating Rewriter session": "Skapar Rewriter-session",
  "Creating Summarizer session": "Skapar sammanfattningssession",
  "Creating Translator session": "Skapar översättningssession",
  "Creating Writer session": "Skapar Writer-session",
  "Deciding capability": "Beslutsförmåga",
  Description: "Beskrivning",
  "Detecting language": "Upptäcka språk",
  "Done.": "Gjort.",
  "Downloading model…": "Laddar ner modell…",
  "Downloading model...": "Laddar ner modell…",
  Dutch: "holländska",
  Edit: "Redigera",
  "Empty response": "Tomt svar",
  English: "Engelska",
  "Enter a search query to start.": "Ange en sökfråga för att börja.",
  Error: "Fel",
  Excerpt: "Utdrag",
  Filters: "Filter",
  Formal: "Formell",
  formal: "formell",
  French: "franska",
  Generate: "Generera",
  "Generate Image Metadata": "Generera bildmetadata",
  "Generate Post Metadata": "Generera inläggsmetadata",
  "Generated content": "Genererat innehåll",
  "Generating text": "Generera text",
  "Generating…": "Genererar…",
  "Generating...": "Genererar…",
  German: "Tyska",
  Headline: "Rubrik",
  headline: "rubrik",
  Hebrew: "Hebreiska",
  Hide: "Dölja",
  Hindi: "Hindi",
  "Hover highlights to see explanations.":
    "Håll muspekaren över markeringarna för att se förklaringar.",
  HTML: "HTML",
  Hungarian: "Ungerska",
  "I'm ready to assist you.": "Jag är redo att hjälpa dig.",
  "Image no longer available": "Bilden är inte längre tillgänglig",
  "Image preview": "Bildförhandsvisning",
  Indonesian: "Indonesiska",
  "Initializing on-device AI…": "Initierar AI på enheten…",
  "Initializing on-device AI...": "Initierar AI på enheten…",
  "Inlining images as base64": "Inline-bilder som base64",
  "Input and output languages cannot be the same.":
    "Inmatnings- och utmatningsspråk kan inte vara desamma.",
  "Input language": "Inmatningsspråk",
  Instructions: "Instruktioner",
  "Interface language": "Gränssnittspråk",
  Italian: "Italienska",
  Japanese: "japanska",
  "Key Points": "Viktiga punkter",
  "key-points": "nyckelpunkter",
  Korean: "Koreanska",
  Length: "Längd",
  Long: "Lång",
  long: "lång",
  Longer: "Längre",
  longer: "längre",
  Markdown: "Markdown",
  Maximize: "Maximera",
  Medium: "Medium",
  medium: "medium",
  "More casual": "Mer avslappnad",
  "More formal": "Mer formellt",
  "more-casual": "mer avslappnad",
  "more-formal": "mer formell",
  Neutral: "Neutral",
  neutral: "neutral",
  No: "Nej",
  "No content generated.": "Inget innehåll genererades.",
  "No issues found. Your text looks great!":
    "Inga problem hittades. Din text ser bra ut!",
  "No overrides": "Inga åsidosättningar",
  Norwegian: "norska",
  "Not sent": "Inte skickat",
  "On-device model ready.": "Klar för modell på enheten.",
  "On-device model ready...": "On-device-modellen är redo...",
  Options: "Alternativ",
  "Output format": "Utdataformat",
  "Output language": "Utmatningsspråk",
  "Plain Text": "Oformaterad text",
  Polish: "Polska",
  Portuguese: "Portugisiska",
  "Powered by": "Drivs av",
  Preview: "Förhandsvisning",
  Proofread: "Korrekturläs",
  "Proofread again": "Korrekturläs igen",
  "Proofread on Backend": "Korrekturläsning på backend",
  Proofreading: "Korrekturläsning",
  "Proofreading part {current}/{total}...":
    "Korrekturläser del {current}/{total}...",
  "Ready.": "Klar.",
  "Received backend response.": "Mottaget backend-svar.",
  "Receiving response...": "Tar emot svar...",
  Record: "Spela in",
  "Record audio": "Spela in ljud",
  "Recorded audio:": "Inspelat ljud:",
  "Recording...": "Spelar in...",
  Reference: "Referens",
  References: "Referenser",
  Regenerate: "Regenerera",
  "Regenerate on Backend": "Regenerera på backend",
  "Reject response": "Avvisa svar",
  "Remove image": "Ta bort bild",
  "Replace with": "Ersätt med",
  "Requesting signed URL": "Begär signerad URL",
  "Response stopped.": "Svaret stoppades.",
  Reset: "Återställ",
  "Reset conversation": "Återställ konversation",
  "Restore size": "Återställ storlek",
  Rewrite: "Skriv om",
  "Rewrite again": "Skriv om igen",
  "Rewrite on Backend": "Omskrivning på backend",
  "Rewriting part {current}/{total}...": "Skriver om del {current}/{total}...",
  "Rewriting text": "Omskrivning av text",
  "Running a script…": "Kör ett skript…",
  "Running on-device...": "Körs på enheten...",
  Russian: "Ryska",
  Search: "Söka",
  "Search the documentation…": "Sök i dokumentationen…",
  "Search with AI-Kit": "Sök med AI-Kit",
  "Searching knowledge base…": "Söker i kunskapsbasen…",
  "Searching…": "Sökande…",
  "Select or type tags…": "Välj eller skriv in taggar…",
  "Select or type tags...": "Välj eller skriv in taggar…",
  Send: "Skicka",
  "Sending request to backend…": "Skickar begäran till backend…",
  "Sending request to server...": "Skickar begäran till servern...",
  Short: "Kort",
  short: "kort",
  Shorter: "Kortare",
  shorter: "kortare",
  Show: "Visa",
  "Sites searched: {count}": "Genomsökta webbplatser: {count}",
  "Something went wrong.": "Något gick fel.",
  "Something went wrong. Please try again.": "Något gick fel. Försök igen.",
  "We couldn't verify that you're human. Please try again.": "Vi kunde inte verifiera att du är en människa. Försök igen.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Du har inte behörighet att använda den här AI-funktionen. Logga in eller kontakta webbplatsägaren.",
  "Too many requests. Please wait a moment and try again.": "För många förfrågningar. Vänta en stund och försök igen.",
  "The request could not be processed. Review your input and try again.": "Begäran kunde inte behandlas. Kontrollera din inmatning och försök igen.",
  "We couldn't reach the AI service. Check your connection and try again.": "Vi kunde inte nå AI-tjänsten. Kontrollera din anslutning och försök igen.",
  "The AI service is temporarily unavailable. Please try again.": "AI-tjänsten är tillfälligt otillgänglig. Försök igen.",
  "Request ID": "Begäran-ID",
  Sources: "Källor",
  Spanish: "Spanska",
  Stop: "Stopp",
  "Stop recording": "Stoppa inspelning",
  Subcategories: "Underkategorier",
  "Suggested change": "Föreslagen ändring",
  Summarize: "Sammanfatta",
  "Summarize again": "Sammanfatta igen",
  "Summarize on Backend": "Sammanfatta på backend",
  Summarizing: "Sammanfattning",
  "Summarizing part {current}/{total}...":
    "Sammanfattar del {current}/{total}...",
  Swedish: "Svenska",
  Tags: "Taggar",
  Teaser: "Teaser",
  teaser: "teaser",
  Thai: "Thailändsk",
  "The alt text for the image.": "Alt-texten för bilden.",
  "The caption for the image.": "Bildtexten till bilden.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "Den konfigurerade AI-modellen stöder inte bildbilagor. Kontakta webbplatsadministratören.",
  "The description for the image.": "Beskrivningen för bilden.",
  "The excerpt for the post.":
    "Valfritt. Kort sammanfattning som kan visas i sökresultat och förhandsvisningar.",
  "The format for the generated output.": "Formatet för den genererade utdata.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "Det språk AI-Kit ska använda för genererad text som standard (när det är tillämpligt).",
  "The language of the input text.": "Språket för inmatningstexten.",
  "The summary style to generate.": "Sammanfattningsstilen som ska genereras.",
  "The target output length.": "Målutgångslängden.",
  "The title for the image.": "Titeln för bilden.",
  "The title for the post.": "Titeln för inlägget.",
  "The tone or style for the AI to use.":
    "Tonen eller stilen som AI:n ska använda.",
  "The topic or subject for the AI to write about.":
    "Ämnet eller ämnet som AI:n ska skriva om.",
  "Thinking…": "Tänker…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Den här sammanfattningen genererades av AI och kan innehålla fel. Kontrollera viktig information.",
  Title: "Titel",
  "TL;DR": "TL;DR",
  tldr: "tdr",
  Tone: "Ton",
  Topic: "Ämne",
  Translate: "Översätt",
  "Translate again": "Översätt igen",
  "Translate on Backend": "Översätt på backend",
  Translating: "Översätter",
  "Translating text": "Översätter text",
  "Translating part {current}/{total}...":
    "Översätter del {current}/{total}...",
  Turkish: "turkiska",
  Type: "Typ",
  Ukrainian: "ukrainska",
  "Unexpected error": "Oväntat fel",
  Uploading: "Uppladdning",
  "Uploading images via signed URL": "Ladda upp bilder via signerad URL",
  User: "Användare",
  "View image": "Visa bild",
  "Using backend": "Använda backend",
  "Using on-device": "Använda på enheten",
  "Waiting for backend response…": "Väntar på svar från backend-systemet…",
  "Waiting for response...": "Väntar på svar...",
  Why: "Varför",
  "Working…": "Arbetar…",
  "{name} is working…": "{name} arbetar…",
  "Generating response…": "Genererar svar…",
  "Knowledge Base searches: {count}": "Sökningar i kunskapsbasen: {count}",
  "Websites searched: {count}": "Genomsökta webbplatser: {count}",
  "Other tools used: {count}": "Andra verktyg som använts: {count}",
  "Working...": "Arbetar…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Skriv",
  "Writing part {current}/{total}...": "Skriver del {current}/{total}...",
  Yes: "Ja",
  "You need a subscription to use this feature on frontend.":
    "Du behöver en prenumeration för att använda den här funktionen på frontend.",
};

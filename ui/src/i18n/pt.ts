export const ptDict: Record<string, string> = {
  "{name} is thinking…": "{name} está pensando…",
  "Sending request…": "Enviando solicitação…",
  "Waiting for reply…": "Aguardando resposta…",
  "Reconnecting…": "Reconectando…",
  "Checking sources…": "Verificando fontes…",
  "Knowledge base searched": "Base de conhecimento consultada",
  "Web search completed": "Pesquisa na web concluída",
  "Calculation completed": "Cálculo concluído",
  "Script completed": "Script concluído",
  "Check completed": "Verificação concluída",
  "Action completed": "Ação concluída",
  "Action failed": "Falha na ação",
  "Search query: {query}": "Consulta de pesquisa: {query}",
  "Categories: {count}": "Categorias: {count}",
  "Subcategories: {count}": "Subcategorias: {count}",
  "Tags: {count}": "Etiquetas: {count}",
  "Results: {count}": "Resultados: {count}",
  "Retry": "Tentar novamente",
  "The answer could not be verified against its cited sources. Please try again.": "Não foi possível verificar a resposta com base nas fontes citadas. Tente novamente.",
  "The knowledge base search could not be completed. Please try again.": "Não foi possível concluir a pesquisa na base de conhecimento. Tente novamente.",
  "The answer's source check could not be completed. Please try again.": "Não foi possível concluir a verificação das fontes da resposta. Tente novamente.",
  "Action interrupted": "Ação interrompida",
  "Failed actions: {count}": "Ações com falha: {count}",
  "--- Select ---": "--- Selecionar ---",
  "A response is still running. Wait for it to finish or cancel it.": "Ainda está a ser gerada uma resposta. Aguarde até terminar ou cancele-a.",
  Accept: "Aceitar",
  "Accept response": "Aceitar resposta",
  Add: "Adicionar",
  "Add image": "Adicionar imagem",
  "Additional instructions to guide the AI.":
    "Instruções adicionais para orientar a IA.",
  "AI Assistant": "Assistente de IA",
  "AI Summary": "Resumo de IA",
  "Alt Text": "Texto alternativo",
  "An error occurred.": "Ocorreu um erro.",
  "An unknown error occurred.": "Ocorreu um erro desconhecido.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "As respostas de {name} são geradas por IA e podem conter erros. Verifique informações importantes.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Não encontrei informações fiáveis suficientes para responder. Reformule a pergunta ou delimite melhor o tema.",
  "The AI response took too long. Please try again.":
    "A resposta da IA demorou demasiado tempo. Tente novamente.",
  Arabic: "árabe",
  "Are you sure you want to reset the conversation?":
    "Tem a certeza de que pretende repor a conversa?",
  "As-Is": "Sem alterações",
  "as-is": "tal como está",
  "Ask anything…": "Pergunte qualquer coisa…",
  "Ask me": "Pergunte-me",
  Assistant: "Assistente",
  "Assistant is thinking…": "O assistente está pensando…",
  "Audio message": "Mensagem de áudio",
  "Audio no longer available": "O áudio já não está disponível",
  "Audio recorded": "Áudio gravado",
  auto: "Automático",
  "Auto-detect": "Detecção automática",
  "Calculating…": "A calcular…",
  Cancel: "Cancelar",
  Caption: "Rubrica",
  Casual: "Casual",
  casual: "casual",
  Categories: "Categorias",
  "Checking…": "A verificar…",
  "Checking capabilities…": "Verificando as capacidades…",
  "Checking capabilities...": "Verificando as capacidades…",
  Chinese: "chinês",
  Clear: "Limpar",
  "Clear audio": "Limpar áudio",
  "Click again to confirm": "Clique novamente para confirmar",
  Close: "Fechar",
  "Close chat": "Fechar chat",
  Confirm: "Confirmar",
  Corrected: "Corrigido",
  "Creating final summary...": "Criando resumo final...",
  "Creating LanguageDetector session": "Criando sessão do LanguageDetector",
  "Creating LanguageModel session": "Sessão de criação do modelo de linguagem",
  "Creating Proofreader session": "Sessão de criação de revisão ortográfica",
  "Creating Rewriter session": "Sessão de criação de reescritor",
  "Creating Summarizer session": "Sessão de criação do Summarizer",
  "Creating Translator session": "Sessão de criação de tradutor",
  "Creating Writer session": "Sessão de Criação de Escritores",
  "Deciding capability": "Capacidade de decisão",
  Description: "Descrição",
  "Detecting language": "Detecção de linguagem",
  "Done.": "Feito.",
  "Downloading model…": "Baixando modelo…",
  "Downloading model...": "Baixando modelo…",
  Dutch: "Holandês",
  Edit: "Editar",
  "Empty response": "Resposta vazia",
  English: "Inglês",
  "Enter a search query to start.": "Insira um termo de pesquisa para começar.",
  Error: "Erro",
  Excerpt: "Trecho",
  Filters: "Filtros",
  Formal: "Formal",
  formal: "formal",
  French: "Francês",
  Generate: "Gerar",
  "Generate Image Metadata": "Gerar metadados da imagem",
  "Generate Post Metadata": "Gerar metadados do post",
  "Generated content": "Conteúdo gerado",
  "Generating text": "Geração de texto",
  "Generating…": "Gerando…",
  "Generating...": "Gerando…",
  German: "Alemão",
  Headline: "Título",
  headline: "título",
  Hebrew: "hebraico",
  Hide: "Esconder",
  Hindi: "hindi",
  "Hover highlights to see explanations.":
    "Passe o cursor sobre os destaques para ver as explicações.",
  HTML: "HTML",
  Hungarian: "húngaro",
  "I'm ready to assist you.": "Estou pronto para ajudar.",
  "Image no longer available": "A imagem já não está disponível",
  "Image preview": "Pré-visualização da imagem",
  Indonesian: "indonésio",
  "Initializing on-device AI…": "Inicializando IA no dispositivo…",
  "Initializing on-device AI...": "Inicializando IA no dispositivo…",
  "Inlining images as base64": "Incorporação de imagens em base64",
  "Input and output languages cannot be the same.":
    "As linguagens de entrada e saída não podem ser as mesmas.",
  "Input language": "Língua de entrada",
  Instructions: "Instruções",
  "Interface language": "Idioma da interface",
  Italian: "italiano",
  Japanese: "japonês",
  "Key Points": "Pontos-chave",
  "key-points": "pontos-chave",
  Korean: "coreano",
  Length: "Comprimento",
  Long: "Longo",
  long: "longo",
  Longer: "Mais longo",
  longer: "mais longo",
  Markdown: "Markdown",
  Maximize: "Maximizar",
  Medium: "Médio",
  medium: "médio",
  "More casual": "Mais casual",
  "More formal": "Mais formal",
  "more-casual": "mais informal",
  "more-formal": "mais formal",
  Neutral: "Neutro",
  neutral: "neutro",
  No: "Não",
  "No content generated.": "Nenhum conteúdo gerado.",
  "No issues found. Your text looks great!":
    "Nenhum problema encontrado. Seu texto está ótimo!",
  "No overrides": "Sem substituições",
  Norwegian: "norueguês",
  "Not sent": "Não enviado",
  "On-device model ready.": "Modelo pronto para uso no dispositivo.",
  "On-device model ready...": "Modelo no dispositivo pronto...",
  Options: "Opções",
  "Output format": "Formato de saída",
  "Output language": "Idioma de saída",
  "Plain Text": "Texto simples",
  Polish: "polonês",
  Portuguese: "Português",
  "Powered by": "Distribuído por",
  Preview: "Pré-visualização",
  Proofread: "Rever",
  "Proofread again": "Revisar novamente",
  "Proofread on Backend": "Revisão no Backend",
  Proofreading: "Revisão",
  "Proofreading part {current}/{total}...":
    "Revisando a parte {current}/{total}...",
  "Ready.": "Pronto.",
  "Received backend response.": "Resposta do servidor recebida.",
  "Receiving response...": "A receber resposta...",
  Record: "Gravar",
  "Record audio": "Gravar áudio",
  "Recorded audio:": "Áudio gravado:",
  "Recording...": "A gravar...",
  Reference: "Referência",
  References: "Referências",
  Regenerate: "Gerar novamente",
  "Regenerate on Backend": "Regenerar no backend",
  "Reject response": "Rejeitar resposta",
  "Remove image": "Remover imagem",
  "Replace with": "Substitua por",
  "Requesting signed URL": "Solicitação de URL assinada",
  "Response stopped.": "Resposta interrompida.",
  Reset: "Repor",
  "Reset conversation": "Repor conversa",
  "Restore size": "Restaurar tamanho",
  Rewrite: "Reescrever",
  "Rewrite again": "Reescrever novamente",
  "Rewrite on Backend": "Reescrever no backend",
  "Rewriting part {current}/{total}...":
    "Reescrevendo a parte {current}/{total}...",
  "Rewriting text": "Reescrevendo o texto",
  "Running a script…": "A executar um script…",
  "Running on-device...": "A executar no dispositivo...",
  Russian: "russo",
  Search: "Procurar",
  "Search the documentation…": "Pesquise a documentação…",
  "Search with AI-Kit": "Pesquisar com AI-Kit",
  "Searching knowledge base…": "A pesquisar na base de conhecimento…",
  "Searching…": "Procurando…",
  "Select or type tags…": "Selecione ou digite tags…",
  "Select or type tags...": "Selecione ou digite tags…",
  Send: "Enviar",
  "Sending request to backend…": "Enviando solicitação ao servidor…",
  "Sending request to server...": "A enviar pedido ao servidor...",
  Short: "Curto",
  short: "curto",
  Shorter: "Mais curto",
  shorter: "mais curto",
  Show: "Mostrar",
  "Sites searched: {count}": "Sites consultados: {count}",
  "Something went wrong.": "Algo deu errado.",
  "Something went wrong. Please try again.": "Algo deu errado. Tente novamente.",
  "We couldn't verify that you're human. Please try again.": "Não foi possível verificar se você é humano. Tente novamente.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Você não está autorizado a usar este recurso de IA. Faça login ou entre em contato com o proprietário do site.",
  "Too many requests. Please wait a moment and try again.": "Muitas solicitações. Aguarde um momento e tente novamente.",
  "The request could not be processed. Review your input and try again.": "Não foi possível processar a solicitação. Revise sua entrada e tente novamente.",
  "We couldn't reach the AI service. Check your connection and try again.": "Não foi possível acessar o serviço de IA. Verifique sua conexão e tente novamente.",
  "The AI service is temporarily unavailable. Please try again.": "O serviço de IA está temporariamente indisponível. Tente novamente.",
  "Request ID": "ID da solicitação",
  Sources: "Fontes",
  Spanish: "Espanhol",
  Stop: "Parar",
  "Stop recording": "Parar gravação",
  Subcategories: "Subcategorias",
  "Suggested change": "Alteração sugerida",
  Summarize: "Resumir",
  "Summarize again": "Resuma novamente",
  "Summarize on Backend": "Resumo no Backend",
  Summarizing: "Resumindo",
  "Summarizing part {current}/{total}...":
    "Resumindo a parte {current}/{total}...",
  Swedish: "sueco",
  Tags: "Tags",
  Teaser: "Teaser",
  teaser: "teaser",
  Thai: "Tailandês",
  "The alt text for the image.": "O texto alternativo da imagem.",
  "The caption for the image.": "Legenda da imagem.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "O modelo de IA configurado não suporta anexos de imagem. Contacte o administrador do site.",
  "The description for the image.": "Descrição da imagem.",
  "The excerpt for the post.":
    "Opcional. Um breve resumo que pode aparecer nos resultados de pesquisa e nas pré-visualizações.",
  "The format for the generated output.": "O formato da saída gerada.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "O idioma que o AI-Kit deve usar por padrão para o texto gerado (quando aplicável).",
  "The language of the input text.": "O idioma do texto de entrada.",
  "The summary style to generate.": "O estilo de resumo a ser gerado.",
  "The target output length.": "O comprimento de saída desejado.",
  "The title for the image.": "O título da imagem.",
  "The title for the post.": "Título da postagem.",
  "The tone or style for the AI to use.": "O tom ou estilo que a IA deve usar.",
  "The topic or subject for the AI to write about.":
    "O tema ou assunto sobre o qual a IA deve escrever.",
  "Thinking…": "A pensar…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Este resumo foi gerado por IA e pode conter erros. Verifique informações importantes.",
  Title: "Título",
  "TL;DR": "Resumindo:",
  tldr: "Resumindo",
  Tone: "Tom",
  Topic: "Tópico",
  Translate: "Traduzir",
  "Translate again": "Traduzir novamente",
  "Translate on Backend": "Traduzir no Backend",
  Translating: "Traduzindo",
  "Translating text": "Traduzindo texto",
  "Translating part {current}/{total}...":
    "Traduzindo a parte {current}/{total}...",
  Turkish: "turco",
  Type: "Tipo",
  Ukrainian: "ucraniano",
  "Unexpected error": "Erro inesperado",
  Uploading: "Carregando",
  "Uploading images via signed URL": "Carregar imagens através de URL assinada",
  User: "Utilizador",
  "View image": "Ver imagem",
  "Using backend": "Usando o backend",
  "Using on-device": "Usando o dispositivo",
  "Waiting for backend response…": "Aguardando resposta do servidor…",
  "Waiting for response...": "A aguardar resposta...",
  Why: "Por que",
  "Working…": "A trabalhar…",
  "{name} is working…": "{name} está a trabalhar…",
  "Generating response…": "A gerar resposta…",
  "Knowledge Base searches: {count}": "Pesquisas na base de conhecimento: {count}",
  "Websites searched: {count}": "Sites consultados: {count}",
  "Other tools used: {count}": "Outras ferramentas usadas: {count}",
  "Working...": "A trabalhar…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Escrever",
  "Writing part {current}/{total}...":
    "Escrevendo a parte {current}/{total}...",
  Yes: "Sim",
  "You need a subscription to use this feature on frontend.":
    "Você precisa de uma assinatura para usar esse recurso no frontend.",
};

export const plDict: Record<string, string> = {
  "{name} is thinking…": "{name} myśli…",
  "Sending request…": "Wysyłanie żądania…",
  "Waiting for reply…": "Oczekiwanie na odpowiedź…",
  "Reconnecting…": "Ponowne łączenie…",
  "Checking sources…": "Sprawdzanie źródeł…",
  "Knowledge base searched": "Przeszukano bazę wiedzy",
  "Web search completed": "Wyszukiwanie w sieci zakończone",
  "Calculation completed": "Obliczenia zakończone",
  "Script completed": "Wykonanie skryptu zakończone",
  "Check completed": "Sprawdzanie zakończone",
  "Action completed": "Działanie zakończone",
  "Action failed": "Działanie nie powiodło się",
  "Search query: {query}": "Zapytanie: {query}",
  "Categories: {count}": "Kategorie: {count}",
  "Subcategories: {count}": "Podkategorie: {count}",
  "Tags: {count}": "Tagi: {count}",
  "Results: {count}": "Wyniki: {count}",
  "Retry": "Spróbuj ponownie",
  "The answer could not be verified against its cited sources. Please try again.": "Nie udało się zweryfikować odpowiedzi na podstawie cytowanych źródeł. Proszę spróbować ponownie.",
  "The knowledge base search could not be completed. Please try again.": "Nie udało się ukończyć wyszukiwania w bazie wiedzy. Proszę spróbować ponownie.",
  "The answer's source check could not be completed. Please try again.": "Nie udało się ukończyć weryfikacji źródeł odpowiedzi. Proszę spróbować ponownie.",
  "Action interrupted": "Działanie przerwane",
  "Failed actions: {count}": "Nieudane działania: {count}",
  "--- Select ---": "--- Wybierz ---",
  "A response is still running. Wait for it to finish or cancel it.": "Odpowiedź jest nadal generowana. Poczekaj na jej zakończenie albo ją anuluj.",
  Accept: "Akceptuj",
  "Accept response": "Zaakceptuj odpowiedź",
  Add: "Dodaj",
  "Add image": "Dodaj obraz",
  "Additional instructions to guide the AI.":
    "Dodatkowe instrukcje dotyczące sterowania sztuczną inteligencją.",
  "AI Assistant": "Asystent AI",
  "AI Summary": "Podsumowanie sztucznej inteligencji",
  "Alt Text": "Tekst alternatywny",
  "An error occurred.": "Wystąpił błąd.",
  "An unknown error occurred.": "Wystąpił nieznany błąd.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Odpowiedzi od {name} są generowane przez AI i mogą zawierać błędy. Sprawdź ważne informacje.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Nie udało się znaleźć wystarczająco wiarygodnych informacji, aby odpowiedzieć. Proszę przeformułować pytanie lub zawęzić temat.",
  "The AI response took too long. Please try again.":
    "Odpowiedź AI trwała zbyt długo. Proszę spróbować ponownie.",
  Arabic: "arabski",
  "Are you sure you want to reset the conversation?":
    "Czy na pewno chcesz zresetować rozmowę?",
  "As-Is": "Bez zmian",
  "as-is": "tak jak jest",
  "Ask anything…": "Zapytaj o cokolwiek…",
  "Ask me": "Zapytaj mnie",
  Assistant: "Asystent",
  "Assistant is thinking…": "Asystent myśli…",
  "Audio message": "Wiadomość audio",
  "Audio no longer available": "Audio nie jest już dostępne",
  "Audio recorded": "Audio nagrano",
  auto: "automatyczny",
  "Auto-detect": "Automatyczne wykrywanie",
  "Calculating…": "Obliczanie…",
  Cancel: "Anulować",
  Caption: "Podpis",
  Casual: "Nieformalny",
  casual: "zwykły",
  Categories: "Kategorie",
  "Checking…": "Sprawdzanie…",
  "Checking capabilities…": "Sprawdzanie możliwości…",
  "Checking capabilities...": "Sprawdzanie możliwości…",
  Chinese: "chiński",
  Clear: "Wyczyść",
  "Clear audio": "Wyczyść audio",
  "Click again to confirm": "Kliknij ponownie, aby potwierdzić",
  Close: "Zamknąć",
  "Close chat": "Zamknij czat",
  Confirm: "Potwierdź",
  Corrected: "Poprawiono",
  "Creating final summary...": "Tworzenie końcowego podsumowania...",
  "Creating LanguageDetector session": "Tworzenie sesji LanguageDetector",
  "Creating LanguageModel session": "Tworzenie sesji LanguageModel",
  "Creating Proofreader session": "Tworzenie sesji korektora",
  "Creating Rewriter session": "Tworzenie sesji Rewritera",
  "Creating Summarizer session": "Tworzenie sesji Summarizera",
  "Creating Translator session": "Tworzenie sesji Tłumacza",
  "Creating Writer session": "Tworzenie sesji Writer",
  "Deciding capability": "Decydowanie o zdolności",
  Description: "Opis",
  "Detecting language": "Wykrywanie języka",
  "Done.": "Zrobione.",
  "Downloading model…": "Pobieranie modelu…",
  "Downloading model...": "Pobieranie modelu…",
  Dutch: "Holenderski",
  Edit: "Edytuj",
  "Empty response": "Pusta odpowiedź",
  English: "angielski",
  "Enter a search query to start.":
    "Aby rozpocząć, wprowadź zapytanie wyszukiwania.",
  Error: "Błąd",
  Excerpt: "Fragment",
  Filters: "Filtry",
  Formal: "Formalny",
  formal: "formalny",
  French: "francuski",
  Generate: "Generuj",
  "Generate Image Metadata": "Generuj metadane obrazu",
  "Generate Post Metadata": "Generuj metadane wpisu",
  "Generated content": "Wygenerowana treść",
  "Generating text": "Generowanie tekstu",
  "Generating…": "Generowanie…",
  "Generating...": "Generowanie…",
  German: "niemiecki",
  Headline: "Nagłówek",
  headline: "nagłówek",
  Hebrew: "hebrajski",
  Hide: "Ukrywać",
  Hindi: "hinduski",
  "Hover highlights to see explanations.":
    "Najedź kursorem na wyróżnienia, aby zobaczyć wyjaśnienia.",
  HTML: "HTML",
  Hungarian: "węgierski",
  "I'm ready to assist you.": "Jestem gotowy, aby Ci pomóc.",
  "Image no longer available": "Obraz nie jest już dostępny",
  "Image preview": "Podgląd obrazu",
  Indonesian: "indonezyjski",
  "Initializing on-device AI…":
    "Inicjalizacja sztucznej inteligencji na urządzeniu…",
  "Initializing on-device AI...":
    "Inicjalizacja sztucznej inteligencji na urządzeniu…",
  "Inlining images as base64": "Wstawianie obrazów w formacie base64",
  "Input and output languages cannot be the same.":
    "Języki wejściowe i wyjściowe nie mogą być takie same.",
  "Input language": "Język wejściowy",
  Instructions: "Instrukcje",
  "Interface language": "Język interfejsu",
  Italian: "włoski",
  Japanese: "japoński",
  "Key Points": "Kluczowe punkty",
  "key-points": "kluczowe punkty",
  Korean: "koreański",
  Length: "Długość",
  Long: "Długi",
  long: "długi",
  Longer: "Dłużej",
  longer: "dłużej",
  Markdown: "Markdown",
  Maximize: "Maksymalizuj",
  Medium: "Średni",
  medium: "średni",
  "More casual": "Bardziej swobodny",
  "More formal": "Bardziej formalny",
  "more-casual": "bardziej swobodny",
  "more-formal": "bardziej formalny",
  Neutral: "Neutralny",
  neutral: "neutralny",
  No: "Nie",
  "No content generated.": "Nie wygenerowano treści.",
  "No issues found. Your text looks great!":
    "Nie znaleziono problemów. Twój tekst wygląda świetnie!",
  "No overrides": "Brak nadpisań",
  Norwegian: "norweski",
  "Not sent": "Nie wysłano",
  "On-device model ready.": "Model gotowy do instalacji na urządzeniu.",
  "On-device model ready...": "Model na urządzeniu gotowy...",
  Options: "Opcje",
  "Output format": "Format wyjściowy",
  "Output language": "Język wyjściowy",
  "Plain Text": "Zwykły tekst",
  Polish: "Polski",
  Portuguese: "portugalski",
  "Powered by": "Zasilane przez",
  Preview: "Zapowiedź",
  Proofread: "Korekta",
  "Proofread again": "Sprawdź jeszcze raz",
  "Proofread on Backend": "Korekta w zapleczu",
  Proofreading: "Korekta",
  "Proofreading part {current}/{total}...":
    "Korekta części {current}/{total}...",
  "Ready.": "Gotowe.",
  "Received backend response.": "Otrzymano odpowiedź z zaplecza.",
  "Receiving response...": "Odbieranie odpowiedzi...",
  Record: "Nagraj",
  "Record audio": "Nagraj audio",
  "Recorded audio:": "Nagrane audio:",
  "Recording...": "Nagrywanie...",
  Reference: "Źródło",
  References: "Źródła",
  Regenerate: "Wygeneruj ponownie",
  "Regenerate on Backend": "Regeneruj na zapleczu",
  "Reject response": "Odrzuć odpowiedź",
  "Remove image": "Usuń obraz",
  "Replace with": "Zastąp przez",
  "Requesting signed URL": "Żądanie podpisanego adresu URL",
  "Response stopped.": "Odpowiedź zatrzymana.",
  Reset: "Resetuj",
  "Reset conversation": "Zresetuj rozmowę",
  "Restore size": "Przywróć rozmiar",
  Rewrite: "Przepisz",
  "Rewrite again": "Napisz jeszcze raz",
  "Rewrite on Backend": "Przepisanie zaplecza",
  "Rewriting part {current}/{total}...":
    "Przepisywanie części {current}/{total}...",
  "Rewriting text": "Przepisywanie tekstu",
  "Running a script…": "Uruchamianie skryptu…",
  "Running on-device...": "Uruchamianie na urządzeniu...",
  Russian: "rosyjski",
  Search: "Szukaj",
  "Search the documentation…": "Przeszukaj dokumentację…",
  "Search with AI-Kit": "Szukaj za pomocą AI-Kit",
  "Searching knowledge base…": "Przeszukiwanie bazy wiedzy…",
  "Searching…": "Badawczy…",
  "Select or type tags…": "Wybierz lub wpisz tagi…",
  "Select or type tags...": "Wybierz lub wpisz tagi…",
  Send: "Wyślij",
  "Sending request to backend…": "Wysyłanie żądania do zaplecza…",
  "Sending request to server...": "Wysyłanie żądania do serwera...",
  Short: "Krótki",
  short: "krótki",
  Shorter: "Krótszy",
  shorter: "krótszy",
  Show: "Pokazywać",
  "Sites searched: {count}": "Przeszukane witryny: {count}",
  "Something went wrong.": "Coś poszło nie tak.",
  "Something went wrong. Please try again.": "Coś poszło nie tak. Spróbuj ponownie.",
  "We couldn't verify that you're human. Please try again.": "Nie udało się zweryfikować, że jesteś człowiekiem. Spróbuj ponownie.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Nie masz uprawnień do korzystania z tej funkcji AI. Zaloguj się lub skontaktuj się z właścicielem witryny.",
  "Too many requests. Please wait a moment and try again.": "Zbyt wiele żądań. Poczekaj chwilę i spróbuj ponownie.",
  "The request could not be processed. Review your input and try again.": "Nie można przetworzyć żądania. Sprawdź dane wejściowe i spróbuj ponownie.",
  "We couldn't reach the AI service. Check your connection and try again.": "Nie można połączyć się z usługą AI. Sprawdź połączenie i spróbuj ponownie.",
  "The AI service is temporarily unavailable. Please try again.": "Usługa AI jest tymczasowo niedostępna. Spróbuj ponownie.",
  "Request ID": "Identyfikator żądania",
  Sources: "Źródła",
  Spanish: "hiszpański",
  Stop: "Zatrzymywać się",
  "Stop recording": "Zatrzymaj nagrywanie",
  Subcategories: "Podkategorie",
  "Suggested change": "Sugerowana zmiana",
  Summarize: "Streszczać",
  "Summarize again": "Podsumuj jeszcze raz",
  "Summarize on Backend": "Podsumowanie na zapleczu",
  Summarizing: "Zreasumowanie",
  "Summarizing part {current}/{total}...":
    "Tworzenie podsumowania części {current}/{total}...",
  Swedish: "szwedzki",
  Tags: "Tagi",
  Teaser: "Zajawka",
  teaser: "kłopotliwe pytanie",
  Thai: "tajski",
  "The alt text for the image.": "Tekst alternatywny dla obrazka.",
  "The caption for the image.": "Podpis pod obrazkiem.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "Skonfigurowany model AI nie obsługuje załączników graficznych. Skontaktuj się z administratorem witryny.",
  "The description for the image.": "Opis obrazu.",
  "The excerpt for the post.":
    "Opcjonalne. Krótkie podsumowanie, które może pojawić się w wynikach wyszukiwania i podglądach.",
  "The format for the generated output.": "Format wygenerowanego wyniku.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "Język, którego AI-Kit powinien domyślnie używać w generowanym tekście (jeśli ma to zastosowanie).",
  "The language of the input text.": "Język tekstu wejściowego.",
  "The summary style to generate.": "Styl podsumowania do wygenerowania.",
  "The target output length.": "Docelowa długość wyjściowa.",
  "The title for the image.": "Tytuł obrazu.",
  "The title for the post.": "Tytuł postu.",
  "The tone or style for the AI to use.":
    "Ton lub styl, którego ma używać sztuczna inteligencja.",
  "The topic or subject for the AI to write about.":
    "Temat lub przedmiot, o którym będzie pisać sztuczna inteligencja.",
  "Thinking…": "Myślenie…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "To podsumowanie zostało wygenerowane przez AI i może zawierać błędy. Sprawdź ważne informacje.",
  Title: "Tytuł",
  "TL;DR": "Krótko mówiąc",
  tldr: "tldr",
  Tone: "Ton",
  Topic: "Temat",
  Translate: "Przetłumacz",
  "Translate again": "Przetłumacz jeszcze raz",
  "Translate on Backend": "Przetłumacz na zapleczu",
  Translating: "Tłumaczenie",
  "Translating text": "Tłumaczenie tekstu",
  "Translating part {current}/{total}...":
    "Tłumaczenie części {current}/{total}...",
  Turkish: "turecki",
  Type: "Typ",
  Ukrainian: "ukraiński",
  "Unexpected error": "Nieoczekiwany błąd",
  Uploading: "Przesyłanie",
  "Uploading images via signed URL":
    "Przesyłanie obrazów za pomocą podpisanego adresu URL",
  User: "Użytkownik",
  "View image": "Wyświetl obraz",
  "Using backend": "Korzystanie z zaplecza",
  "Using on-device": "Korzystanie z urządzenia",
  "Waiting for backend response…": "Oczekiwanie na odpowiedź zaplecza…",
  "Waiting for response...": "Oczekiwanie na odpowiedź...",
  Why: "Dlaczego",
  "Working…": "Pracuję…",
  "{name} is working…": "{name} pracuje…",
  "Generating response…": "Generowanie odpowiedzi…",
  "Knowledge Base searches: {count}": "Wyszukiwania w bazie wiedzy: {count}",
  "Websites searched: {count}": "Przeszukane witryny: {count}",
  "Other tools used: {count}": "Inne użyte narzędzia: {count}",
  "Working...": "Pracuję…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Napisz",
  "Writing part {current}/{total}...": "Pisanie części {current}/{total}...",
  Yes: "Tak",
  "You need a subscription to use this feature on frontend.":
    "Aby korzystać z tej funkcji w interfejsie użytkownika, wymagana jest subskrypcja.",
};

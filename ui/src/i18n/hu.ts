export const huDict: Record<string, string> = {
  "{name} is thinking…": "{name} gondolkodik…",
  "Sending request…": "Kérés küldése…",
  "Waiting for reply…": "Várakozás a válaszra…",
  "Reconnecting…": "Újracsatlakozás…",
  "Checking sources…": "Források ellenőrzése…",
  "Knowledge base searched": "Tudástárkeresés befejezve",
  "Web search completed": "Webes keresés befejezve",
  "Calculation completed": "Számítás befejezve",
  "Script completed": "Szkript futtatása befejezve",
  "Check completed": "Ellenőrzés befejezve",
  "Action completed": "Művelet befejezve",
  "Action failed": "Sikertelen művelet",
  "Search query: {query}": "Keresőkifejezés: {query}",
  "Categories: {count}": "Kategóriák: {count}",
  "Subcategories: {count}": "Alkategóriák: {count}",
  "Tags: {count}": "Címkék: {count}",
  "Results: {count}": "Találatok: {count}",
  "Retry": "Újrapróbálás",
  "The answer could not be verified against its cited sources. Please try again.": "A választ nem sikerült ellenőrizni a hivatkozott források alapján. Kérjük, próbálja újra.",
  "The knowledge base search could not be completed. Please try again.": "A tudástárkeresés nem fejeződött be. Kérjük, próbálja újra.",
  "The answer's source check could not be completed. Please try again.": "A válasz forrásainak ellenőrzése nem fejeződött be. Kérjük, próbálja újra.",
  "Action interrupted": "Megszakított művelet",
  "Failed actions: {count}": "Sikertelen műveletek: {count}",
  "--- Select ---": "--- Válasszon ---",
  "A response is still running. Wait for it to finish or cancel it.": "Még készül egy válasz. Várja meg, amíg befejeződik, vagy szakítsa meg.",
  Accept: "Elfogad",
  "Accept response": "Válasz elfogadása",
  Add: "Hozzáadás",
  "Add image": "Kép hozzáadása",
  "Additional instructions to guide the AI.":
    "További utasítások a modell irányításához.",
  "AI Assistant": "AI asszisztens",
  "AI Summary": "AI-összefoglaló",
  "Alt Text": "Alt szöveg",
  "An error occurred.": "Hiba történt.",
  "An unknown error occurred.": "Ismeretlen hiba történt.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "A {name} válaszait mesterséges intelligencia állítja elő, ezért hibákat tartalmazhatnak. Ellenőrizze a fontos információkat.",
  Arabic: "arab",
  "Are you sure you want to reset the conversation?":
    "Biztosan alaphelyzetbe állítja a beszélgetést?",
  "As-Is": "Változatlan",
  "as-is": "változatlan",
  "Ask anything…": "Kérdezzen bármit…",
  "Ask me": "Kérdezzen tőlem",
  Assistant: "Asszisztens",
  "Assistant is thinking…": "Az asszisztens gondolkodik…",
  "Audio message": "Hangüzenet",
  "Audio no longer available": "A hanganyag már nem érhető el",
  "Audio recorded": "Hang rögzítve",
  auto: "Automatikus",
  "Auto-detect": "Automatikus felismerés",
  "Calculating…": "Számolás…",
  Cancel: "Mégse",
  Caption: "Felirat",
  Casual: "Közvetlen",
  casual: "közvetlen",
  Categories: "Kategóriák",
  "Checking…": "Ellenőrzés…",
  "Checking capabilities…": "Képességek ellenőrzése…",
  "Checking capabilities...": "Képességek ellenőrzése…",
  Chinese: "kínai",
  Clear: "Törlés",
  "Clear audio": "Hang törlése",
  "Click again to confirm": "Kattintson újra a megerősítéshez",
  Close: "Bezár",
  "Close chat": "Chat bezárása",
  Confirm: "Megerősítés",
  Corrected: "Javítva",
  "Creating final summary...": "Végső összefoglaló készítése...",
  "Creating LanguageDetector session": "Inicializálás",
  "Creating LanguageModel session": "Inicializálás",
  "Creating Proofreader session": "Inicializálás",
  "Creating Rewriter session": "Inicializálás",
  "Creating Summarizer session": "Inicializálás",
  "Creating Translator session": "Inicializálás",
  "Creating Writer session": "Inicializálás",
  "Deciding capability": "Képességek ellenőrzése",
  Description: "Leírás",
  "Detecting language": "Nyelvfelismerés",
  "Done.": "Kész.",
  "Downloading model…": "Modell letöltése…",
  "Downloading model...": "Modell letöltése…",
  Dutch: "holland",
  Edit: "Szerkesztés",
  "Empty response": "Üres válasz",
  English: "angol",
  "Enter a search query to start.":
    "Írjon be egy keresőkifejezést a kezdéshez.",
  Error: "Hiba történt",
  Excerpt: "Kivonat",
  Filters: "Szűrők",
  Formal: "Hivatalos",
  formal: "hivatalos",
  French: "francia",
  Generate: "Generálás",
  "Generate Image Metadata": "Kép metaadatok generálása",
  "Generate Post Metadata": "Bejegyzés-metaadatok generálása",
  "Generated content": "Generált tartalom",
  "Generating text": "Szöveg generálása",
  "Generating…": "Generálás…",
  "Generating...": "Generálás…",
  German: "német",
  Headline: "Címsor",
  headline: "főcím",
  Hebrew: "héber",
  Hide: "Elrejt",
  Hindi: "hindi",
  "Hover highlights to see explanations.":
    "Vigye az egérmutatót a kiemelésekre a magyarázatok megtekintéséhez.",
  HTML: "HTML",
  Hungarian: "magyar",
  "I'm ready to assist you.": "Készen állok segíteni.",
  "Image no longer available": "A kép már nem érhető el",
  "Image preview": "Kép előnézete",
  Indonesian: "indonéz",
  "Initializing on-device AI…": "Az eszközön lévő modell inicializálása…",
  "Initializing on-device AI...": "Az eszközön lévő modell inicializálása…",
  "Inlining images as base64": "Képfeldolgozás",
  "Input and output languages cannot be the same.":
    "A bemeneti és kimeneti nyelvek nem lehetnek azonosak.",
  "Input language": "Beviteli nyelv",
  Instructions: "Utasítások",
  "Interface language": "Kezelőfelület nyelve",
  Italian: "olasz",
  Japanese: "japán",
  "Key Points": "Főbb pontok",
  "key-points": "kulcsfontosságú pontok",
  Korean: "koreai",
  Length: "Hossz",
  Long: "Hosszú",
  long: "hosszú",
  Longer: "Hosszabb",
  longer: "hosszabb",
  Markdown: "Markdown",
  Maximize: "Maximalizálás",
  Medium: "Közepes",
  medium: "közepes",
  "More casual": "Közvetlenebb",
  "More formal": "Hivatalosabb",
  "more-casual": "közvetlenebb",
  "more-formal": "hivatalosabb",
  Neutral: "Semleges",
  neutral: "semleges",
  No: "Nem",
  "No content generated.": "Nem készült tartalom.",
  "No issues found. Your text looks great!":
    "Nem találtunk problémát. Az Ön szövege megfelelő.",
  "No overrides": "Nincs felülbírálás",
  Norwegian: "norvég",
  "Not sent": "Nincs elküldve",
  "On-device model ready.": "Készen áll az eszközön lévő modell.",
  "On-device model ready...": "Az eszközön futó modell kész...",
  Options: "Opciók",
  "Output format": "Kimeneti formátum",
  "Output language": "Kimeneti nyelv",
  "Plain Text": "Egyszerű szöveg",
  Polish: "lengyel",
  Portuguese: "portugál",
  "Powered by": "Működteti:",
  Preview: "Előnézet",
  Proofread: "Szövegellenőrzés",
  "Proofread again": "Szövegellenőrzés újra",
  "Proofread on Backend": "Szövegellenőrzés a szerveren",
  Proofreading: "Szövegellenőrzés",
  "Proofreading part {current}/{total}...":
    "{current}/{total}. rész lektorálása...",
  "Ready.": "Kész.",
  "Received backend response.": "Szerverválasz érkezett.",
  "Receiving response...": "Válasz fogadása...",
  Record: "Felvétel",
  "Record audio": "Hang rögzítése",
  "Recorded audio:": "Rögzített hang:",
  "Recording...": "Rögzítés...",
  Reference: "Hivatkozás",
  References: "Hivatkozások",
  Regenerate: "Újragenerálás",
  "Regenerate on Backend": "Újragenerálás a szerveren",
  "Reject response": "Válasz elutasítása",
  "Remove image": "Kép eltávolítása",
  "Replace with": "Cserélje erre",
  "Requesting signed URL": "Aláírt URL kérése",
  "Response stopped.": "A válasz leállítva.",
  Reset: "Visszaállítás",
  "Reset conversation": "Beszélgetés alaphelyzetbe állítása",
  "Restore size": "Méret visszaállítása",
  Rewrite: "Átírás",
  "Rewrite again": "Írja át újra",
  "Rewrite on Backend": "Átírás a szerveren",
  "Rewriting part {current}/{total}...": "{current}/{total}. rész átírása...",
  "Rewriting text": "Szöveg átírása",
  "Running a script…": "Szkript futtatása…",
  "Running on-device...": "Eszközön fut...",
  Russian: "orosz",
  Search: "Keresés",
  "Search the documentation…": "Keresés a dokumentációban…",
  "Search with AI-Kit": "Keresés AI-Kittel",
  "Searching knowledge base…": "Keresés a tudásbázisban…",
  "Searching…": "Keresés…",
  "Select or type tags…": "Válasszon vagy írjon be címkéket…",
  "Select or type tags...": "Válasszon vagy írjon be címkéket…",
  Send: "Küldés",
  "Sending request to backend…": "Kérés küldése a szervernek…",
  "Sending request to server...": "Kérés küldése a szervernek...",
  Short: "Rövid",
  short: "rövid",
  Shorter: "Rövidebb",
  shorter: "rövidebb",
  Show: "Megjelenítés",
  "Sites searched: {count}": "Átnézett webhelyek: {count}",
  "Something went wrong.": "Valami baj történt.",
  "Something went wrong. Please try again.":
    "Valami baj történt. Próbálja újra.",
  "We couldn't verify that you're human. Please try again.":
    "Nem sikerült ellenőrizni, hogy Ön emberi felhasználó-e. Próbálja újra.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.":
    "Ön nem jogosult ennek az AI-funkciónak a használatára. Jelentkezzen be, vagy forduljon a webhely gazdájához.",
  "Too many requests. Please wait a moment and try again.":
    "Túl sok kérés érkezett. Várjon egy kicsit, majd próbálja újra.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Nem található elegendő megbízható információ a válaszhoz. Fogalmazza át a kérdést, vagy szűkítse a témát.",
  "The AI response took too long. Please try again.":
    "Az AI-válaszra túl sokat kellett várni. Próbálja újra.",
  "The request could not be processed. Review your input and try again.":
    "A kérést nem sikerült feldolgozni. Ellenőrizze a bevitt adatokat, majd próbálja újra.",
  "We couldn't reach the AI service. Check your connection and try again.":
    "Az AI-szolgáltatás nem érhető el. Ellenőrizze a kapcsolatot, majd próbálja újra.",
  "The AI service is temporarily unavailable. Please try again.":
    "Az AI-szolgáltatás átmenetileg nem érhető el. Próbálja újra.",
  "Request ID": "Kérésazonosító",
  Sources: "Források",
  Spanish: "spanyol",
  Stop: "Leállítás",
  "Stop recording": "Rögzítés leállítása",
  Subcategories: "Alkategóriák",
  "Suggested change": "Javasolt módosítás",
  Summarize: "Összefoglalás",
  "Summarize again": "Foglalja össze újra",
  "Summarize on Backend": "Összefoglalás a szerveren",
  Summarizing: "Összefoglalás",
  "Summarizing part {current}/{total}...":
    "{current}/{total}. rész összegzése...",
  Swedish: "svéd",
  Tags: "Címkék",
  Teaser: "Előzetes",
  teaser: "előzetes",
  Thai: "thai",
  "The alt text for the image.": "A képhez tartozó alt szöveg.",
  "The caption for the image.": "A kép felirata.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "A beállított AI-modell nem támogatja a képek csatolását. Forduljon a webhely adminisztrátorához.",
  "The description for the image.": "A kép leírása.",
  "The excerpt for the post.":
    "Opcionális. Rövid összefoglaló, amely megjelenhet a keresési találatokban és az előnézetekben.",
  "The format for the generated output.": "A generált kimenet formátuma.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "Az AI-Kit által alapértelmezés szerint a generált szöveghez használt nyelv (ha alkalmazható).",
  "The language of the input text.": "A bemeneti szöveg nyelve.",
  "The summary style to generate.": "A létrehozandó összefoglaló stílus.",
  "The target output length.": "A cél kimeneti hossza.",
  "The title for the image.": "A kép címe.",
  "The title for the post.": "A bejegyzés címe.",
  "The tone or style for the AI to use.":
    "A modell által használandó hangnem vagy stílus.",
  "The topic or subject for the AI to write about.":
    "A téma vagy tárgykör, amelyről a modell ír.",
  "Thinking…": "Gondolkodás…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Ezt az összefoglalót mesterséges intelligencia állította össze, ezért hibákat tartalmazhat. Ellenőrizze a fontos információkat.",
  Title: "Cím",
  "TL;DR": "TL;DR",
  tldr: "tldr",
  Tone: "Hangnem",
  Topic: "Téma",
  Translate: "Fordítás",
  "Translate again": "Fordítás újra",
  "Translate on Backend": "Fordítás a szerveren",
  Translating: "Fordítás",
  "Translating text": "Szöveg fordítása",
  "Translating part {current}/{total}...":
    "{current}/{total}. rész fordítása...",
  Turkish: "török",
  Type: "Típus",
  Ukrainian: "ukrán",
  "Unexpected error": "Váratlan hiba",
  Uploading: "Feltöltés folyamatban",
  "Uploading images via signed URL": "Képek feltöltése aláírt URL-en keresztül",
  User: "Felhasználó",
  "View image": "Kép megtekintése",
  "Using backend": "Szerver használata",
  "Using on-device": "Eszközön belüli használat",
  "Waiting for backend response…": "Várakozás a szerver válaszára…",
  "Waiting for response...": "Várakozás a válaszra...",
  Why: "Miért",
  "Working…": "Feldolgozás…",
  "{name} is working…": "{name} dolgozik…",
  "Generating response…": "Válasz megfogalmazása…",
  "Knowledge Base searches: {count}": "Tudásbázis-keresések: {count}",
  "Websites searched: {count}": "Átnézett webhelyek: {count}",
  "Other tools used: {count}": "Egyéb eszközhasználatok: {count}",
  "Working...": "Feldolgozás…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Írás",
  "Writing part {current}/{total}...": "{current}/{total}. rész írása...",
  Yes: "Igen",
  "You need a subscription to use this feature on frontend.":
    "Előfizetés szükséges a funkció használatához a frontenden.",
};

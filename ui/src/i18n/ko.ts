export const koDict: Record<string, string> = {
  "{name} is thinking…": "{name} 생각 중…",
  "Sending request…": "요청 전송 중…",
  "Waiting for reply…": "응답 대기 중…",
  "Reconnecting…": "다시 연결 중…",
  "Checking sources…": "출처 확인 중…",
  "Knowledge base searched": "지식 기반 검색 완료",
  "Web search completed": "웹 검색 완료",
  "Calculation completed": "계산 완료",
  "Script completed": "스크립트 실행 완료",
  "Check completed": "확인 완료",
  "Action completed": "작업 완료",
  "Action failed": "작업 실패",
  "Search query: {query}": "검색어: {query}",
  "Categories: {count}": "카테고리: {count}",
  "Subcategories: {count}": "하위 카테고리: {count}",
  "Tags: {count}": "태그: {count}",
  "Results: {count}": "결과: {count}",
  "Retry": "다시 시도",
  "The answer could not be verified against its cited sources. Please try again.": "인용된 출처를 기준으로 답변을 검증하지 못했습니다. 다시 시도해 주세요.",
  "The knowledge base search could not be completed. Please try again.": "지식 기반 검색을 완료하지 못했습니다. 다시 시도해 주세요.",
  "The answer's source check could not be completed. Please try again.": "답변의 출처 확인을 완료하지 못했습니다. 다시 시도해 주세요.",
  "Action interrupted": "작업 중단",
  "Failed actions: {count}": "실패한 작업: {count}",
  "--- Select ---": "--- 선택 ---",
  "A response is still running. Wait for it to finish or cancel it.": "아직 응답을 생성하고 있습니다. 완료될 때까지 기다리거나 취소하세요.",
  Accept: "수락",
  "Accept response": "응답 수락",
  Add: "추가",
  "Add image": "이미지 추가",
  "Additional instructions to guide the AI.":
    "인공지능을 안내하기 위한 추가 지침입니다.",
  "AI Assistant": "AI 어시스턴트",
  "AI Summary": "AI 요약",
  "Alt Text": "대체 텍스트",
  "An error occurred.": "오류가 발생했습니다.",
  "An unknown error occurred.": "알 수 없는 오류가 발생했습니다.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "{name}의 답변은 AI가 생성하므로 오류가 있을 수 있습니다. 중요한 정보는 확인하세요.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "답변에 필요한 신뢰할 수 있는 정보를 충분히 찾지 못했습니다. 질문을 다시 표현하거나 주제를 좁혀 주세요.",
  "The AI response took too long. Please try again.":
    "AI 응답에 시간이 너무 오래 걸렸습니다. 다시 시도해 주세요.",
  Arabic: "아라비아 말",
  "Are you sure you want to reset the conversation?":
    "대화를 재설정하시겠습니까?",
  "As-Is": "변경 없음",
  "as-is": "있는 그대로",
  "Ask anything…": "무엇이든 물어보세요…",
  "Ask me": "질문하기",
  Assistant: "어시스턴트",
  "Assistant is thinking…": "비서가 생각 중입니다…",
  "Audio message": "오디오 메시지",
  "Audio no longer available": "오디오를 더 이상 사용할 수 없습니다",
  "Audio recorded": "오디오가 녹음되었습니다",
  auto: "자동",
  "Auto-detect": "자동 감지",
  "Calculating…": "계산하는 중…",
  Cancel: "취소",
  Caption: "표제",
  Casual: "친근한",
  casual: "평상복",
  Categories: "카테고리",
  "Checking…": "확인하는 중…",
  "Checking capabilities…": "기능을 확인하는 중…",
  "Checking capabilities...": "기능을 확인하는 중…",
  Chinese: "중국인",
  Clear: "지우기",
  "Clear audio": "오디오 지우기",
  "Click again to confirm": "확인하려면 다시 클릭하세요",
  Close: "닫기",
  "Close chat": "채팅 닫기",
  Confirm: "확인",
  Corrected: "수정됨",
  "Creating final summary...": "최종 요약을 생성하는 중...",
  "Creating LanguageDetector session": "LanguageDetector 세션 생성 중",
  "Creating LanguageModel session": "LanguageModel 세션 생성 중",
  "Creating Proofreader session": "교정자 세션 생성",
  "Creating Rewriter session": "Rewriter 세션 생성",
  "Creating Summarizer session": "요약기 세션 생성",
  "Creating Translator session": "번역기 세션 생성",
  "Creating Writer session": "작가 세션 만들기",
  "Deciding capability": "결정 능력",
  Description: "설명",
  "Detecting language": "언어 감지",
  "Done.": "완료.",
  "Downloading model…": "모델 다운로드 중…",
  "Downloading model...": "모델 다운로드 중…",
  Dutch: "네덜란드 사람",
  Edit: "편집",
  "Empty response": "빈 응답",
  English: "영어",
  "Enter a search query to start.": "검색을 시작하려면 검색어를 입력하세요.",
  Error: "오류",
  Excerpt: "발췌",
  Filters: "필터",
  Formal: "공식적인",
  formal: "공식적인",
  French: "프랑스 국민",
  Generate: "생성",
  "Generate Image Metadata": "이미지 메타데이터 생성",
  "Generate Post Metadata": "게시물 메타데이터 생성",
  "Generated content": "생성된 콘텐츠",
  "Generating text": "텍스트 생성",
  "Generating…": "생성 중…",
  "Generating...": "생성 중…",
  German: "독일 사람",
  Headline: "표제",
  headline: "표제",
  Hebrew: "헤브라이 사람",
  Hide: "숨다",
  Hindi: "힌디 어",
  "Hover highlights to see explanations.":
    "마우스 커서를 하이라이트에 올려놓으면 설명을 볼 수 있습니다.",
  HTML: "HTML",
  Hungarian: "헝가리 인",
  "I'm ready to assist you.": "도와드릴 준비가 되었습니다.",
  "Image no longer available": "이미지를 더 이상 사용할 수 없습니다",
  "Image preview": "이미지 미리보기",
  Indonesian: "인도네시아 인",
  "Initializing on-device AI…": "기기 내 AI 초기화 중…",
  "Initializing on-device AI...": "기기 내 AI 초기화 중…",
  "Inlining images as base64": "이미지를 base64 형식으로 인라인 처리",
  "Input and output languages cannot be the same.":
    "입력 언어와 출력 언어는 서로 다를 수 있습니다.",
  "Input language": "입력 언어",
  Instructions: "지침",
  "Interface language": "인터페이스 언어",
  Italian: "이탈리아 사람",
  Japanese: "일본어",
  "Key Points": "핵심 사항",
  "key-points": "핵심 사항",
  Korean: "한국인",
  Length: "길이",
  Long: "ê¸´",
  long: "ê¸´",
  Longer: "더 길게",
  longer: "더 길게",
  Markdown: "Markdown",
  Maximize: "최대화",
  Medium: "중간",
  medium: "중간",
  "More casual": "좀 더 캐주얼하게",
  "More formal": "좀 더 격식 있는",
  "more-casual": "좀 더 캐주얼한",
  "more-formal": "보다 격식적인",
  Neutral: "중립적",
  neutral: "중립적",
  No: "아니요",
  "No content generated.": "생성된 콘텐츠가 없습니다.",
  "No issues found. Your text looks great!":
    "아무런 문제가 발견되지 않았습니다. 텍스트가 아주 좋습니다!",
  "No overrides": "재정의 없음",
  Norwegian: "노르웨이 인",
  "Not sent": "전송되지 않음",
  "On-device model ready.": "온디바이스 모델 준비 완료.",
  "On-device model ready...": "온디바이스 모델 준비 완료...",
  Options: "옵션",
  "Output format": "출력 형식",
  "Output language": "출력 언어",
  "Plain Text": "일반 텍스트",
  Polish: "광택",
  Portuguese: "포르투갈 인",
  "Powered by": "Powered by",
  Preview: "시사",
  Proofread: "교정",
  "Proofread again": "다시 한번 교정해 보세요",
  "Proofread on Backend": "백엔드에서 교정",
  Proofreading: "교정 중",
  "Proofreading part {current}/{total}...":
    "{current}/{total} 부분을 교정하는 중...",
  "Ready.": "준비됨.",
  "Received backend response.": "백엔드 응답을 받았습니다.",
  "Receiving response...": "응답 수신 중...",
  Record: "녹음",
  "Record audio": "오디오 녹음",
  "Recorded audio:": "녹음된 오디오:",
  "Recording...": "녹음 중...",
  Reference: "참고",
  References: "참고 자료",
  Regenerate: "다시 생성",
  "Regenerate on Backend": "백엔드에서 재생성",
  "Reject response": "응답 거부",
  "Remove image": "이미지 제거",
  "Replace with": "다음으로 교체하세요",
  "Requesting signed URL": "서명된 URL을 요청합니다",
  "Response stopped.": "응답이 중지되었습니다.",
  Reset: "초기화",
  "Reset conversation": "대화 재설정",
  "Restore size": "크기 복원",
  Rewrite: "고쳐 쓰기",
  "Rewrite again": "다시 쓰세요",
  "Rewrite on Backend": "백엔드에서 재작성",
  "Rewriting part {current}/{total}...":
    "{current}/{total} 부분을 다시 작성하는 중...",
  "Rewriting text": "텍스트 다시쓰기",
  "Running a script…": "스크립트 실행 중…",
  "Running on-device...": "기기에서 실행 중...",
  Russian: "러시아인",
  Search: "찾다",
  "Search the documentation…": "문서를 검색하세요…",
  "Search with AI-Kit": "AI-Kit으로 검색",
  "Searching knowledge base…": "지식 베이스 검색 중…",
  "Searching…": "수색…",
  "Select or type tags…": "태그를 선택하거나 입력…",
  "Select or type tags...": "태그를 선택하거나 입력…",
  Send: "보내기",
  "Sending request to backend…": "백엔드로 요청을 보내는 중…",
  "Sending request to server...": "서버로 요청 전송 중...",
  Short: "짧은",
  short: "짧은",
  Shorter: "더 짧은",
  shorter: "더 짧은",
  Show: "보여주다",
  "Sites searched: {count}": "검색한 사이트: {count}개",
  "Something went wrong.": "뭔가 잘못됐어요.",
  "Something went wrong. Please try again.": "문제가 발생했습니다. 다시 시도하세요.",
  "We couldn't verify that you're human. Please try again.": "사람인지 확인할 수 없습니다. 다시 시도하세요.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "이 AI 기능을 사용할 권한이 없습니다. 로그인하거나 사이트 소유자에게 문의하세요.",
  "Too many requests. Please wait a moment and try again.": "요청이 너무 많습니다. 잠시 기다린 후 다시 시도하세요.",
  "The request could not be processed. Review your input and try again.": "요청을 처리할 수 없습니다. 입력 내용을 확인하고 다시 시도하세요.",
  "We couldn't reach the AI service. Check your connection and try again.": "AI 서비스에 연결할 수 없습니다. 연결 상태를 확인하고 다시 시도하세요.",
  "The AI service is temporarily unavailable. Please try again.": "AI 서비스가 일시적으로 사용할 수 없습니다. 다시 시도하세요.",
  "Request ID": "요청 ID",
  Sources: "출처",
  Spanish: "스페인 사람",
  Stop: "멈추다",
  "Stop recording": "녹음 중지",
  Subcategories: "하위 카테고리",
  "Suggested change": "제안된 변경 사항",
  Summarize: "요약하다",
  "Summarize again": "다시 요약해 보세요",
  "Summarize on Backend": "백엔드 요약",
  Summarizing: "요약하자면",
  "Summarizing part {current}/{total}...":
    "{current}/{total} 부분을 요약하는 중...",
  Swedish: "스웨덴어",
  Tags: "태그",
  Teaser: "소개 문구",
  teaser: "괴롭히는 사람",
  Thai: "태국",
  "The alt text for the image.": "이미지의 대체 텍스트입니다.",
  "The caption for the image.": "이미지에 대한 설명입니다.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "구성된 AI 모델은 이미지 첨부를 지원하지 않습니다. 사이트 관리자에게 문의하세요.",
  "The description for the image.": "이미지에 대한 설명입니다.",
  "The excerpt for the post.":
    "선택 사항. 검색 결과와 미리보기에서 표시될 수 있는 짧은 요약입니다.",
  "The format for the generated output.": "생성된 출력 형식.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "AI-Kit이 기본적으로 생성할 텍스트에 사용할 언어입니다(해당되는 경우).",
  "The language of the input text.": "입력 텍스트의 언어입니다.",
  "The summary style to generate.": "생성할 요약 스타일입니다.",
  "The target output length.": "목표 출력 길이.",
  "The title for the image.": "이미지의 제목입니다.",
  "The title for the post.": "게시글 제목입니다.",
  "The tone or style for the AI to use.": "AI가 사용할 어조 또는 스타일.",
  "The topic or subject for the AI to write about.":
    "인공지능이 글을 쓸 주제 또는 소재.",
  "Thinking…": "생각하는 중…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "이 요약은 AI가 생성했으며 오류가 있을 수 있습니다. 중요한 정보는 확인하세요.",
  Title: "제목",
  "TL;DR": "요약",
  tldr: "요약",
  Tone: "톤",
  Topic: "주제",
  Translate: "번역",
  "Translate again": "다시 번역하기",
  "Translate on Backend": "백엔드에서 번역",
  Translating: "번역하기",
  "Translating text": "텍스트 번역 중",
  "Translating part {current}/{total}...":
    "{current}/{total} 부분을 번역하는 중...",
  Turkish: "터키어",
  Type: "유형",
  Ukrainian: "우크라이나 말",
  "Unexpected error": "예상치 못한 오류",
  Uploading: "업로드 중",
  "Uploading images via signed URL": "서명된 URL을 통해 이미지 업로드",
  User: "사용자",
  "View image": "이미지 보기",
  "Using backend": "백엔드 사용",
  "Using on-device": "기기 내에서 사용",
  "Waiting for backend response…": "백엔드 응답을 기다리는 중입니다…",
  "Waiting for response...": "응답 대기 중...",
  Why: "왜",
  "Working…": "작업 중…",
  "{name} is working…": "{name} 작업 중…",
  "Generating response…": "답변 생성 중…",
  "Knowledge Base searches: {count}": "지식 베이스 검색: {count}회",
  "Websites searched: {count}": "검색한 웹사이트: {count}개",
  "Other tools used: {count}": "기타 도구 사용: {count}회",
  "Working...": "작업 중…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "작성",
  "Writing part {current}/{total}...":
    "{current}/{total} 부분을 작성하는 중...",
  Yes: "예",
  "You need a subscription to use this feature on frontend.":
    "프런트엔드에서 이 기능을 사용하려면 구독이 필요합니다.",
};

export const arDict: Record<string, string> = {
  "{name} is thinking…": "{name} يفكر…",
  "Sending request…": "جارٍ إرسال الطلب…",
  "Waiting for reply…": "بانتظار الرد…",
  "Reconnecting…": "جارٍ إعادة الاتصال…",
  "Checking sources…": "جارٍ التحقق من المصادر…",
  "Knowledge base searched": "اكتمل البحث في قاعدة المعرفة",
  "Web search completed": "اكتمل البحث على الويب",
  "Calculation completed": "اكتملت العملية الحسابية",
  "Script completed": "اكتمل تشغيل البرنامج النصي",
  "Check completed": "اكتمل التحقق",
  "Action completed": "اكتمل الإجراء",
  "Action failed": "فشل الإجراء",
  "Search query: {query}": "عبارة البحث: {query}",
  "Categories: {count}": "الفئات: {count}",
  "Subcategories: {count}": "الفئات الفرعية: {count}",
  "Tags: {count}": "الوسوم: {count}",
  "Results: {count}": "النتائج: {count}",
  "Retry": "إعادة المحاولة",
  "The answer could not be verified against its cited sources. Please try again.": "تعذر التحقق من الإجابة بالرجوع إلى المصادر المذكورة. يُرجى إعادة المحاولة.",
  "The knowledge base search could not be completed. Please try again.": "تعذر إكمال البحث في قاعدة المعرفة. يُرجى إعادة المحاولة.",
  "The answer's source check could not be completed. Please try again.": "تعذر إكمال التحقق من مصادر الإجابة. يُرجى إعادة المحاولة.",
  "Action interrupted": "تمت مقاطعة الإجراء",
  "Failed actions: {count}": "الإجراءات الفاشلة: {count}",
  "--- Select ---": "--- اختر ---",
  "A response is still running. Wait for it to finish or cancel it.": "لا تزال هناك إجابة قيد الإنشاء. انتظر حتى تكتمل أو ألغها.",
  Accept: "قبول",
  "Accept response": "قبول الرد",
  Add: "إضافة",
  "Add image": "إضافة صورة",
  "Additional instructions to guide the AI.":
    "تعليمات إضافية لتوجيه الذكاء الاصطناعي.",
  "AI Assistant": "مساعد الذكاء الاصطناعي",
  "AI Summary": "ملخص الذكاء الاصطناعي",
  "Alt Text": "نص بديل",
  "An error occurred.": "حدث خطأ.",
  "An unknown error occurred.": "حدث خطأ غير معروف.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "تُنشأ إجابات {name} بواسطة الذكاء الاصطناعي وقد تتضمن أخطاء. تحقّق من المعلومات المهمة.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "لم أتمكن من العثور على معلومات موثوقة كافية للإجابة. يُرجى إعادة صياغة السؤال أو تحديد الموضوع بدقة أكبر.",
  "The AI response took too long. Please try again.":
    "استغرق رد الذكاء الاصطناعي وقتًا طويلًا جدًا. يُرجى المحاولة مرة أخرى.",
  Arabic: "عربي",
  "Are you sure you want to reset the conversation?":
    "هل أنت متأكد أنك تريد إعادة ضبط المحادثة؟",
  "As-Is": "بدون تغيير",
  "as-is": "كما هي",
  "Ask anything…": "اسأل أي شيء…",
  "Ask me": "اسألني",
  Assistant: "المساعد",
  "Assistant is thinking…": "المساعد يفكر…",
  "Audio message": "رسالة صوتية",
  "Audio no longer available": "الرسالة الصوتية لم تعد متاحة",
  "Audio recorded": "تم تسجيل الصوت",
  auto: "آلي",
  "Auto-detect": "اكتشاف تلقائي",
  "Calculating…": "جارٍ الحساب…",
  Cancel: "يلغي",
  Caption: "التسمية التوضيحية",
  Casual: "غير رسمي",
  casual: "غير رسمي",
  Categories: "الفئات",
  "Checking…": "جارٍ التحقق…",
  "Checking capabilities…": "إمكانيات الفحص…",
  "Checking capabilities...": "إمكانيات الفحص…",
  Chinese: "الصينية",
  Clear: "مسح",
  "Clear audio": "مسح الصوت",
  "Click again to confirm": "انقر مرة أخرى للتأكيد",
  Close: "يغلق",
  "Close chat": "إغلاق الدردشة",
  Confirm: "تأكيد",
  Corrected: "تم التصحيح",
  "Creating final summary...": "إنشاء الملخص النهائي...",
  "Creating LanguageDetector session": "إنشاء جلسة كاشف اللغة",
  "Creating LanguageModel session": "إنشاء جلسة نموذج اللغة",
  "Creating Proofreader session": "إنشاء جلسة مدقق لغوي",
  "Creating Rewriter session": "إنشاء جلسة إعادة الكتابة",
  "Creating Summarizer session": "إنشاء جلسة تلخيص",
  "Creating Translator session": "إنشاء جلسة ترجمة",
  "Creating Writer session": "إنشاء جلسة الكتابة",
  "Deciding capability": "القدرة على اتخاذ القرار",
  Description: "وصف",
  "Detecting language": "الكشف عن اللغة",
  "Done.": "منتهي.",
  "Downloading model…": "جارٍ تنزيل النموذج…",
  "Downloading model...": "جارٍ تنزيل النموذج…",
  Dutch: "هولندي",
  Edit: "تعديل",
  "Empty response": "رد فارغ",
  English: "إنجليزي",
  "Enter a search query to start.": "أدخل عبارة بحث للبدء.",
  Error: "خطأ",
  Excerpt: "مقتطفات",
  Filters: "عوامل التصفية",
  Formal: "رَسمِيّ",
  formal: "رَسمِيّ",
  French: "فرنسي",
  Generate: "إنشاء",
  "Generate Image Metadata": "إنشاء بيانات وصفية للصورة",
  "Generate Post Metadata": "إنشاء بيانات وصفية للمقال",
  "Generated content": "المحتوى المُنشأ",
  "Generating text": "إنشاء نص",
  "Generating…": "جارٍ التوليد…",
  "Generating...": "جارٍ التوليد…",
  German: "الألمانية",
  Headline: "العنوان",
  headline: "عنوان رئيسي",
  Hebrew: "العبرية",
  Hide: "يخفي",
  Hindi: "الهندية",
  "Hover highlights to see explanations.":
    "مرر مؤشر الماوس فوق العناصر المميزة للاطلاع على التفسيرات.",
  HTML: "HTML",
  Hungarian: "المجرية",
  "I'm ready to assist you.": "أنا جاهز لمساعدتك.",
  "Image no longer available": "الصورة لم تعد متاحة",
  "Image preview": "معاينة الصورة",
  Indonesian: "إندونيسيا",
  "Initializing on-device AI…": "جارٍ تهيئة الذكاء الاصطناعي على الجهاز…",
  "Initializing on-device AI...": "جارٍ تهيئة الذكاء الاصطناعي على الجهاز…",
  "Inlining images as base64": "تضمين الصور بصيغة base64",
  "Input and output languages cannot be the same.":
    "لا يمكن أن تكون لغات الإدخال والإخراج متطابقة.",
  "Input language": "لغة الإدخال",
  Instructions: "تعليمات",
  "Interface language": "لغة الواجهة",
  Italian: "إيطالي",
  Japanese: "اليابانية",
  "Key Points": "النقاط الرئيسية",
  "key-points": "النقاط الرئيسية",
  Korean: "كوري",
  Length: "طول",
  Long: "طويل",
  long: "طويل",
  Longer: "أطول",
  longer: "أطول",
  Markdown: "Markdown",
  Maximize: "تكبير",
  Medium: "واسطة",
  medium: "واسطة",
  "More casual": "أكثر عفوية",
  "More formal": "أكثر رسمية",
  "more-casual": "أكثر عفوية",
  "more-formal": "أكثر رسمية",
  Neutral: "حيادي",
  neutral: "حيادي",
  No: "لا",
  "No content generated.": "لم يتم إنشاء أي محتوى.",
  "No issues found. Your text looks great!":
    "لم يتم العثور على أي مشاكل. النص يبدو رائعاً!",
  "No overrides": "لا توجد تجاوزات",
  Norwegian: "النرويجية",
  "Not sent": "لم يتم الإرسال",
  "On-device model ready.": "النموذج جاهز للاستخدام على الجهاز.",
  "On-device model ready...": "النموذج على الجهاز جاهز...",
  Options: "خيارات",
  "Output format": "تنسيق الإخراج",
  "Output language": "لغة الإخراج",
  "Plain Text": "نص عادي",
  Polish: "بولندي",
  Portuguese: "البرتغالية",
  "Powered by": "مدعوم بواسطة",
  Preview: "معاينة",
  Proofread: "التدقيق اللغوي",
  "Proofread again": "راجع النص مرة أخرى",
  "Proofread on Backend": "تدقيق لغوي على الخادم",
  Proofreading: "التدقيق اللغوي",
  "Proofreading part {current}/{total}...":
    "جارٍ تدقيق الجزء {current}/{total}...",
  "Ready.": "جاهز.",
  "Received backend response.": "تم استلام رد من الخادم الخلفي.",
  "Receiving response...": "جارٍ استلام الرد...",
  Record: "تسجيل",
  "Record audio": "تسجيل الصوت",
  "Recorded audio:": "صوت مسجّل:",
  "Recording...": "جارٍ التسجيل...",
  Reference: "مرجع",
  References: "المراجع",
  Regenerate: "تجديد",
  "Regenerate on Backend": "إعادة التوليد في الواجهة الخلفية",
  "Reject response": "رفض الرد",
  "Remove image": "إزالة الصورة",
  "Replace with": "استبدل بـ",
  "Requesting signed URL": "طلب عنوان URL موقّع",
  "Response stopped.": "تم إيقاف الرد.",
  Reset: "إعادة ضبط",
  "Reset conversation": "إعادة ضبط المحادثة",
  "Restore size": "استعادة الحجم",
  Rewrite: "إعادة كتابة",
  "Rewrite again": "أعد الكتابة مرة أخرى",
  "Rewrite on Backend": "إعادة كتابة على الخادم",
  "Rewriting part {current}/{total}...":
    "جارٍ إعادة كتابة الجزء {current}/{total}...",
  "Rewriting text": "إعادة كتابة النص",
  "Running a script…": "جارٍ تشغيل برنامج نصي…",
  "Running on-device...": "جارٍ التشغيل على الجهاز...",
  Russian: "الروسية",
  Search: "يبحث",
  "Search the documentation…": "ابحث في الوثائق…",
  "Search with AI-Kit": "البحث باستخدام مجموعة أدوات الذكاء الاصطناعي",
  "Searching knowledge base…": "جارٍ البحث في قاعدة المعرفة…",
  "Searching…": "جارٍ البحث…",
  "Select or type tags…": "اختر أو اكتب الوسوم…",
  "Select or type tags...": "اختر أو اكتب الوسوم…",
  Send: "إرسال",
  "Sending request to backend…": "إرسال الطلب إلى الواجهة الخلفية…",
  "Sending request to server...": "جارٍ إرسال الطلب إلى الخادم...",
  Short: "قصير",
  short: "قصير",
  Shorter: "أقصر",
  shorter: "أقصر",
  Show: "يعرض",
  "Sites searched: {count}": "المواقع التي تم البحث فيها: {count}",
  "Something went wrong.": "حدث خطأ ما.",
  "Something went wrong. Please try again.": "حدث خطأ ما. حاول مرة أخرى.",
  "We couldn't verify that you're human. Please try again.": "تعذر التحقق من أنك إنسان. حاول مرة أخرى.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "غير مصرح لك باستخدام ميزة الذكاء الاصطناعي هذه. سجّل الدخول أو تواصل مع صاحب الموقع.",
  "Too many requests. Please wait a moment and try again.": "العديد جدًا من الطلبات. يرجى الانتظار لحظة ثم إعادة المحاولة.",
  "The request could not be processed. Review your input and try again.": "تعذر معالجة الطلب. راجع مدخلاتك وحاول مرة أخرى.",
  "We couldn't reach the AI service. Check your connection and try again.": "تعذر الوصول إلى خدمة الذكاء الاصطناعي. تحقق من اتصالك وحاول مرة أخرى.",
  "The AI service is temporarily unavailable. Please try again.": "خدمة الذكاء الاصطناعي غير متاحة مؤقتًا. يرجى المحاولة مرة أخرى.",
  "Request ID": "معرّف الطلب",
  Sources: "مصادر",
  Spanish: "الأسبانية",
  Stop: "قف",
  "Stop recording": "إيقاف التسجيل",
  Subcategories: "الفئات الفرعية",
  "Suggested change": "التغيير المقترح",
  Summarize: "لخص",
  "Summarize again": "أعد التلخيص",
  "Summarize on Backend": "تلخيص على الخادم",
  Summarizing: "ملخص",
  "Summarizing part {current}/{total}...":
    "جارٍ تلخيص الجزء {current}/{total}...",
  Swedish: "السويدية",
  Tags: "الوسوم",
  Teaser: "نص تمهيدي",
  teaser: "إعلان تشويقي",
  Thai: "تايلاندي",
  "The alt text for the image.": "النص البديل للصورة.",
  "The caption for the image.": "التعليق المصاحب للصورة.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "نموذج الذكاء الاصطناعي المُعدّ لا يدعم إرفاق الصور. تواصل مع مسؤول الموقع.",
  "The description for the image.": "وصف الصورة.",
  "The excerpt for the post.":
    "اختياري. ملخص قصير قد يظهر في نتائج البحث والمعاينات.",
  "The format for the generated output.": "تنسيق المخرجات المُنشأة.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "اللغة التي يجب أن يستخدمها برنامج AI-Kit للنصوص المُولّدة افتراضيًا (عند الاقتضاء).",
  "The language of the input text.": "لغة النص المدخل.",
  "The summary style to generate.": "نمط الملخص المراد إنشاؤه.",
  "The target output length.": "طول الإخراج المستهدف.",
  "The title for the image.": "عنوان الصورة.",
  "The title for the post.": "عنوان المنشور.",
  "The tone or style for the AI to use.":
    "النبرة أو الأسلوب الذي سيستخدمه الذكاء الاصطناعي.",
  "The topic or subject for the AI to write about.":
    "الموضوع أو الفكرة التي سيكتب عنها الذكاء الاصطناعي.",
  "Thinking…": "جارٍ التفكير…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "أُنشئ هذا الملخص بواسطة الذكاء الاصطناعي وقد يتضمن أخطاء. تحقّق من المعلومات المهمة.",
  Title: "عنوان",
  "TL;DR": "باختصار شديد",
  tldr: "باختصار",
  Tone: "الأسلوب",
  Topic: "الموضوع",
  Translate: "يترجم",
  "Translate again": "ترجم مرة أخرى",
  "Translate on Backend": "ترجمة على الخادم",
  Translating: "الترجمة",
  "Translating text": "جارٍ ترجمة النص",
  "Translating part {current}/{total}...":
    "جارٍ ترجمة الجزء {current}/{total}...",
  Turkish: "تركي",
  Type: "يكتب",
  Ukrainian: "الأوكرانية",
  "Unexpected error": "خطأ غير متوقع",
  Uploading: "جارٍ التحميل",
  "Uploading images via signed URL": "تحميل الصور عبر رابط موقّع",
  User: "المستخدم",
  "View image": "عرض الصورة",
  "Using backend": "استخدام الواجهة الخلفية",
  "Using on-device": "استخدام الجهاز",
  "Waiting for backend response…": "في انتظار رد من الخادم الخلفي…",
  "Waiting for response...": "جارٍ انتظار الرد...",
  Why: "لماذا",
  "Working…": "جارٍ العمل…",
  "{name} is working…": "{name} قيد العمل…",
  "Generating response…": "جارٍ إنشاء الرد…",
  "Knowledge Base searches: {count}": "عمليات البحث في قاعدة المعرفة: {count}",
  "Websites searched: {count}": "مواقع الويب التي تم البحث فيها: {count}",
  "Other tools used: {count}": "الأدوات الأخرى المستخدمة: {count}",
  "Working...": "جارٍ العمل…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "اكتب",
  "Writing part {current}/{total}...": "جارٍ كتابة الجزء {current}/{total}...",
  Yes: "نعم",
  "You need a subscription to use this feature on frontend.":
    "أنت بحاجة إلى اشتراك لاستخدام هذه الميزة في واجهة المستخدم.",
};

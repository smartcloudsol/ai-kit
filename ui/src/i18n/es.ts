export const esDict: Record<string, string> = {
  "{name} is thinking…": "{name} está pensando…",
  "Sending request…": "Enviando solicitud…",
  "Waiting for reply…": "Esperando respuesta…",
  "Reconnecting…": "Reconectando…",
  "Checking sources…": "Comprobando fuentes…",
  "Knowledge base searched": "Base de conocimientos consultada",
  "Web search completed": "Búsqueda web completada",
  "Calculation completed": "Cálculo completado",
  "Script completed": "Script completado",
  "Check completed": "Comprobación completada",
  "Action completed": "Acción completada",
  "Action failed": "Acción fallida",
  "Search query: {query}": "Consulta de búsqueda: {query}",
  "Categories: {count}": "Categorías: {count}",
  "Subcategories: {count}": "Subcategorías: {count}",
  "Tags: {count}": "Etiquetas: {count}",
  "Results: {count}": "Resultados: {count}",
  "Retry": "Reintentar",
  "The answer could not be verified against its cited sources. Please try again.": "No se pudo verificar la respuesta con las fuentes citadas. Vuelva a intentarlo.",
  "The knowledge base search could not be completed. Please try again.": "No se pudo completar la búsqueda en la base de conocimientos. Vuelva a intentarlo.",
  "The answer's source check could not be completed. Please try again.": "No se pudo completar la comprobación de las fuentes de la respuesta. Vuelva a intentarlo.",
  "Action interrupted": "Acción interrumpida",
  "Failed actions: {count}": "Acciones fallidas: {count}",
  "--- Select ---": "--- Seleccionar ---",
  "A response is still running. Wait for it to finish or cancel it.": "Todavía se está generando una respuesta. Espere a que termine o cancélela.",
  Accept: "Aceptar",
  "Accept response": "Aceptar respuesta",
  Add: "Añadir",
  "Add image": "Añadir imagen",
  "Additional instructions to guide the AI.":
    "Instrucciones adicionales para guiar a la IA.",
  "AI Assistant": "Asistente de IA",
  "AI Summary": "Resumen de IA",
  "Alt Text": "Texto alternativo",
  "An error occurred.": "Se produjo un error.",
  "An unknown error occurred.": "Se produjo un error desconocido.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Las respuestas de {name} son generadas por IA y pueden contener errores. Verifique la información importante.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "No he encontrado información fiable suficiente para responder. Reformule la pregunta o delimite el tema.",
  "The AI response took too long. Please try again.":
    "La respuesta de la IA tardó demasiado. Inténtelo de nuevo.",
  Arabic: "árabe",
  "Are you sure you want to reset the conversation?":
    "¿Seguro que quiere restablecer la conversación?",
  "As-Is": "Sin cambios",
  "as-is": "tal cual",
  "Ask anything…": "Haga cualquier pregunta…",
  "Ask me": "Pregúnteme",
  Assistant: "Asistente",
  "Assistant is thinking…": "El asistente está pensando…",
  "Audio message": "Mensaje de audio",
  "Audio no longer available": "El audio ya no está disponible",
  "Audio recorded": "Audio grabado",
  auto: "Automático",
  "Auto-detect": "Detección automática",
  "Calculating…": "Calculando…",
  Cancel: "Cancelar",
  Caption: "Pie de foto",
  Casual: "Informal",
  casual: "casual",
  Categories: "Categorías",
  "Checking…": "Comprobando…",
  "Checking capabilities…": "Comprobando capacidades…",
  "Checking capabilities...": "Comprobando capacidades…",
  Chinese: "Chino",
  Clear: "Borrar",
  "Clear audio": "Borrar audio",
  "Click again to confirm": "Haga clic de nuevo para confirmar",
  Close: "Cerrar",
  "Close chat": "Cerrar chat",
  Confirm: "Confirmar",
  Corrected: "Corregido",
  "Creating final summary...": "Creando resumen final...",
  "Creating LanguageDetector session": "Creando una sesión de LanguageDetector",
  "Creating LanguageModel session": "Creación de una sesión de LanguageModel",
  "Creating Proofreader session":
    "Creación de una sesión de corrección de pruebas",
  "Creating Rewriter session": "Creando una sesión de Rewriter",
  "Creating Summarizer session": "Creación de una sesión de resumen",
  "Creating Translator session": "Creando una sesión de traductor",
  "Creating Writer session": "Creando una sesión de escritor",
  "Deciding capability": "Capacidad de decisión",
  Description: "Descripción",
  "Detecting language": "Detectando el idioma",
  "Done.": "Hecho.",
  "Downloading model…": "Descargando modelo…",
  "Downloading model...": "Descargando modelo…",
  Dutch: "Holandés",
  Edit: "Editar",
  "Empty response": "Respuesta vacía",
  English: "Inglés",
  "Enter a search query to start.":
    "Introduzca una consulta de búsqueda para comenzar.",
  Error: "Error",
  Excerpt: "Extracto",
  Filters: "Filtros",
  Formal: "Formal",
  formal: "formal",
  French: "Francés",
  Generate: "Generar",
  "Generate Image Metadata": "Generar metadatos de imagen",
  "Generate Post Metadata": "Generar metadatos de la entrada",
  "Generated content": "Contenido generado",
  "Generating text": "Generando texto",
  "Generating…": "Generando…",
  "Generating...": "Generando…",
  German: "Alemán",
  Headline: "Titular",
  headline: "titular",
  Hebrew: "hebreo",
  Hide: "Esconder",
  Hindi: "hindi",
  "Hover highlights to see explanations.":
    "Pase el cursor sobre los aspectos destacados para ver las explicaciones.",
  HTML: "HTML",
  Hungarian: "húngaro",
  "I'm ready to assist you.": "Estoy listo para ayudarle.",
  "Image no longer available": "La imagen ya no está disponible",
  "Image preview": "Vista previa de la imagen",
  Indonesian: "indonesio",
  "Initializing on-device AI…": "Inicializando IA en el dispositivo…",
  "Initializing on-device AI...": "Inicializando IA en el dispositivo…",
  "Inlining images as base64": "Inserción de imágenes en base64",
  "Input and output languages cannot be the same.":
    "Los idiomas de entrada y salida no pueden ser el mismo.",
  "Input language": "Idioma de entrada",
  Instructions: "Instrucciones",
  "Interface language": "Idioma de la interfaz",
  Italian: "italiano",
  Japanese: "japonés",
  "Key Points": "Puntos clave",
  "key-points": "puntos clave",
  Korean: "coreano",
  Length: "Longitud",
  Long: "Largo",
  long: "largo",
  Longer: "Más extenso",
  longer: "más extenso",
  Markdown: "Markdown",
  Maximize: "Maximizar",
  Medium: "Medio",
  medium: "medio",
  "More casual": "Más informal",
  "More formal": "Más formal",
  "more-casual": "más informal",
  "more-formal": "más formal",
  Neutral: "Neutral",
  neutral: "neutral",
  No: "No",
  "No content generated.": "No se generó contenido.",
  "No issues found. Your text looks great!":
    "No se encontraron problemas. Su texto está listo.",
  "No overrides": "Sin sobrescrituras",
  Norwegian: "noruego",
  "Not sent": "No enviado",
  "On-device model ready.": "El modelo del dispositivo está listo.",
  "On-device model ready...": "Modelo en el dispositivo listo...",
  Options: "Opciones",
  "Output format": "Formato de salida",
  "Output language": "Idioma de salida",
  "Plain Text": "Texto sin formato",
  Polish: "Polaco",
  Portuguese: "portugués",
  "Powered by": "Con tecnología de",
  Preview: "Vista previa",
  Proofread: "Corregir",
  "Proofread again": "Revisar nuevamente",
  "Proofread on Backend": "Corrección en el backend",
  Proofreading: "Corrección",
  "Proofreading part {current}/{total}...":
    "Corrigiendo la parte {current}/{total}...",
  "Ready.": "Listo.",
  "Received backend response.": "Se recibió una respuesta del backend.",
  "Receiving response...": "Recibiendo respuesta...",
  Record: "Grabar",
  "Record audio": "Grabar audio",
  "Recorded audio:": "Audio grabado:",
  "Recording...": "Grabando...",
  Reference: "Referencia",
  References: "Referencias",
  Regenerate: "Regenerar",
  "Regenerate on Backend": "Regenerar en el backend",
  "Reject response": "Rechazar respuesta",
  "Remove image": "Eliminar imagen",
  "Replace with": "Reemplazar con",
  "Requesting signed URL": "Solicitando URL firmada",
  "Response stopped.": "Respuesta detenida.",
  Reset: "Restablecer",
  "Reset conversation": "Restablecer conversación",
  "Restore size": "Restaurar tamaño",
  Rewrite: "Volver a escribir",
  "Rewrite again": "Reescribir de nuevo",
  "Rewrite on Backend": "Reescribir en el backend",
  "Rewriting part {current}/{total}...":
    "Reescribiendo la parte {current}/{total}...",
  "Rewriting text": "Reescritura de texto",
  "Running a script…": "Ejecutando un script…",
  "Running on-device...": "Ejecutándose en el dispositivo...",
  Russian: "ruso",
  Search: "Buscar",
  "Search the documentation…": "Buscar en la documentación…",
  "Search with AI-Kit": "Buscar con AI-Kit",
  "Searching knowledge base…": "Buscando en la base de conocimientos…",
  "Searching…": "Buscando…",
  "Select or type tags…": "Seleccione o escriba etiquetas…",
  "Select or type tags...": "Seleccione o escriba etiquetas…",
  Send: "Enviar",
  "Sending request to backend…": "Enviando solicitud al backend…",
  "Sending request to server...": "Enviando solicitud al servidor...",
  Short: "Corto",
  short: "corto",
  Shorter: "Más corto",
  shorter: "más corto",
  Show: "Mostrar",
  "Sites searched: {count}": "Sitios consultados: {count}",
  "Something went wrong.": "Algo salió mal.",
  "Something went wrong. Please try again.": "Algo salió mal. Inténtelo de nuevo.",
  "We couldn't verify that you're human. Please try again.": "No pudimos verificar que sea una persona. Inténtelo de nuevo.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "No tiene autorización para usar esta función de IA. Inicie sesión o contacte con el propietario del sitio.",
  "Too many requests. Please wait a moment and try again.": "Demasiadas solicitudes. Espere un momento y vuelva a intentarlo.",
  "The request could not be processed. Review your input and try again.": "No se pudo procesar la solicitud. Revise su entrada e inténtelo de nuevo.",
  "We couldn't reach the AI service. Check your connection and try again.": "No pudimos conectarnos con el servicio de IA. Revise su conexión e inténtelo de nuevo.",
  "The AI service is temporarily unavailable. Please try again.": "El servicio de IA no está disponible temporalmente. Inténtelo de nuevo.",
  "Request ID": "ID de solicitud",
  Sources: "Fuentes",
  Spanish: "Español",
  Stop: "Detener",
  "Stop recording": "Detener grabación",
  Subcategories: "Subcategorías",
  "Suggested change": "Cambio sugerido",
  Summarize: "Resumir",
  "Summarize again": "Resumir de nuevo",
  "Summarize on Backend": "Resumir en el backend",
  Summarizing: "Resumiendo",
  "Summarizing part {current}/{total}...":
    "Resumiendo la parte {current}/{total}...",
  Swedish: "sueco",
  Tags: "Etiquetas",
  Teaser: "Texto teaser",
  teaser: "texto de avance",
  Thai: "tailandés",
  "The alt text for the image.": "El texto alternativo de la imagen.",
  "The caption for the image.": "El pie de foto de la imagen.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "El modelo de IA configurado no admite archivos adjuntos de imagen. Contacte con el administrador del sitio.",
  "The description for the image.": "La descripción de la imagen.",
  "The excerpt for the post.":
    "Opcional. Un resumen breve que puede mostrarse en los resultados de búsqueda y en las vistas previas.",
  "The format for the generated output.": "El formato de la salida generada.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "El idioma que AI-Kit debe usar para el texto generado de forma predeterminada (cuando corresponda).",
  "The language of the input text.": "El idioma del texto de entrada.",
  "The summary style to generate.": "El estilo de resumen a generar.",
  "The target output length.": "La longitud de salida de destino.",
  "The title for the image.": "El título de la imagen.",
  "The title for the post.": "El título del post.",
  "The tone or style for the AI to use.":
    "El tono o estilo que utilizará la IA.",
  "The topic or subject for the AI to write about.":
    "El tema o asunto sobre el cual escribirá la IA.",
  "Thinking…": "Pensando…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Este resumen fue generado por IA y puede contener errores. Verifique la información importante.",
  Title: "Título",
  "TL;DR": "Resumen",
  tldr: "Resumen",
  Tone: "Tono",
  Topic: "Tema",
  Translate: "Traducir",
  "Translate again": "Traducir de nuevo",
  "Translate on Backend": "Traducir en el backend",
  Translating: "Traducción",
  "Translating text": "Traduciendo texto",
  "Translating part {current}/{total}...":
    "Traduciendo la parte {current}/{total}...",
  Turkish: "turco",
  Type: "Tipo",
  Ukrainian: "ucraniano",
  "Unexpected error": "Error inesperado",
  Uploading: "Subiendo",
  "Uploading images via signed URL": "Subir imágenes a través de URL firmada",
  User: "Usuario",
  "View image": "Ver imagen",
  "Using backend": "Usando backend",
  "Using on-device": "Uso en el dispositivo",
  "Waiting for backend response…": "Esperando la respuesta del backend…",
  "Waiting for response...": "Esperando respuesta...",
  Why: "Por qué",
  "Working…": "Trabajando…",
  "{name} is working…": "{name} está trabajando…",
  "Generating response…": "Generando respuesta…",
  "Knowledge Base searches: {count}": "Búsquedas en la base de conocimientos: {count}",
  "Websites searched: {count}": "Sitios web consultados: {count}",
  "Other tools used: {count}": "Otras herramientas utilizadas: {count}",
  "Working...": "Trabajando…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Escribir",
  "Writing part {current}/{total}...":
    "Escribiendo la parte {current}/{total}...",
  Yes: "Sí",
  "You need a subscription to use this feature on frontend.":
    "Necesita una suscripción para utilizar esta función en la interfaz.",
};

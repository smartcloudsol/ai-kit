export const nbDict: Record<string, string> = {
  "{name} is thinking…": "{name} tenker…",
  "Sending request…": "Sender forespørsel…",
  "Waiting for reply…": "Venter på svar…",
  "Reconnecting…": "Kobler til på nytt…",
  "Checking sources…": "Kontrollerer kilder…",
  "Knowledge base searched": "Kunnskapsbasen er søkt gjennom",
  "Web search completed": "Nettsøk fullført",
  "Calculation completed": "Beregning fullført",
  "Script completed": "Skript fullført",
  "Check completed": "Kontroll fullført",
  "Action completed": "Handling fullført",
  "Action failed": "Handlingen mislyktes",
  "Search query: {query}": "Søkeuttrykk: {query}",
  "Categories: {count}": "Kategorier: {count}",
  "Subcategories: {count}": "Underkategorier: {count}",
  "Tags: {count}": "Etiketter: {count}",
  "Results: {count}": "Resultater: {count}",
  "Retry": "Prøv igjen",
  "The answer could not be verified against its cited sources. Please try again.": "Svaret kunne ikke bekreftes ut fra de oppgitte kildene. Prøv igjen.",
  "The knowledge base search could not be completed. Please try again.": "Søket i kunnskapsbasen kunne ikke fullføres. Prøv igjen.",
  "The answer's source check could not be completed. Please try again.": "Kontrollen av svarets kilder kunne ikke fullføres. Prøv igjen.",
  "Action interrupted": "Handling avbrutt",
  "Failed actions: {count}": "Mislykkede handlinger: {count}",
  "--- Select ---": "--- Velg ---",
  "A response is still running. Wait for it to finish or cancel it.": "Et svar er fortsatt underveis. Vent til det er ferdig, eller avbryt det.",
  Accept: "Godta",
  "Accept response": "Godta svar",
  Add: "Legg til",
  "Add image": "Legg til bilde",
  "Additional instructions to guide the AI.":
    "Ytterligere instruksjoner for å veilede AI-en.",
  "AI Assistant": "AI-assistent",
  "AI Summary": "AI-sammendrag",
  "Alt Text": "Alternativ tekst",
  "An error occurred.": "En feil oppstod.",
  "An unknown error occurred.": "Det oppsto en ukjent feil.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Svar fra {name} genereres av KI og kan inneholde feil. Kontroller viktig informasjon.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Jeg fant ikke nok pålitelig informasjon til å svare på det. Formuler spørsmålet på nytt eller avgrens emnet.",
  "The AI response took too long. Please try again.":
    "KI-svaret tok for lang tid. Prøv igjen.",
  Arabic: "Arabisk",
  "Are you sure you want to reset the conversation?":
    "Er du sikker på at du vil tilbakestille samtalen?",
  "As-Is": "Uendret",
  "as-is": "som den er",
  "Ask anything…": "Spør om hva som helst…",
  "Ask me": "Spør meg",
  Assistant: "Assistent",
  "Assistant is thinking…": "Assistenten tenker…",
  "Audio message": "Lydmelding",
  "Audio no longer available": "Lyd er ikke lenger tilgjengelig",
  "Audio recorded": "Lyd tatt opp",
  auto: "Automatisk",
  "Auto-detect": "Oppdag automatisk",
  "Calculating…": "Beregner…",
  Cancel: "Kansellere",
  Caption: "Tekst",
  Casual: "Uformell",
  casual: "tilfeldig",
  Categories: "Kategorier",
  "Checking…": "Sjekker…",
  "Checking capabilities…": "Sjekker muligheter…",
  "Checking capabilities...": "Sjekker muligheter…",
  Chinese: "kinesisk",
  Clear: "Tøm",
  "Clear audio": "Fjern lyd",
  "Click again to confirm": "Klikk igjen for å bekrefte",
  Close: "Lukke",
  "Close chat": "Lukk chatten",
  Confirm: "Bekreft",
  Corrected: "Korrigert",
  "Creating final summary...": "Oppretter endelig oppsummering...",
  "Creating LanguageDetector session": "Oppretter LanguageDetector-økt",
  "Creating LanguageModel session": "Oppretter LanguageModel-økt",
  "Creating Proofreader session": "Oppretter korrekturleserøkt",
  "Creating Rewriter session": "Oppretter Rewriter-økt",
  "Creating Summarizer session": "Oppretter en oppsummeringsøkt",
  "Creating Translator session": "Oppretter oversetterøkt",
  "Creating Writer session": "Oppretter Writer-økt",
  "Deciding capability": "Beslutningsevne",
  Description: "Beskrivelse",
  "Detecting language": "Oppdager språk",
  "Done.": "Ferdig.",
  "Downloading model…": "Laster ned modell…",
  "Downloading model...": "Laster ned modell…",
  Dutch: "Nederlandsk",
  Edit: "Rediger",
  "Empty response": "Tomt svar",
  English: "Engelsk",
  "Enter a search query to start.": "Skriv inn et søkeord for å starte.",
  Error: "Feil",
  Excerpt: "Utdrag",
  Filters: "Filtre",
  Formal: "Formell",
  formal: "formell",
  French: "Fransk",
  Generate: "Generer",
  "Generate Image Metadata": "Generer bildemetadata",
  "Generate Post Metadata": "Generer innleggsmetadata",
  "Generated content": "Generert innhold",
  "Generating text": "Generering av tekst",
  "Generating…": "Genererer…",
  "Generating...": "Genererer…",
  German: "Tysk",
  Headline: "Overskrift",
  headline: "overskrift",
  Hebrew: "Hebraisk",
  Hide: "Gjemme",
  Hindi: "Hindi",
  "Hover highlights to see explanations.":
    "Hold musepekeren over markeringene for å se forklaringer.",
  HTML: "HTML",
  Hungarian: "Ungarsk",
  "I'm ready to assist you.": "Jeg er klar til å hjelpe deg.",
  "Image no longer available": "Bildet er ikke lenger tilgjengelig",
  "Image preview": "Forhåndsvisning av bilde",
  Indonesian: "Indonesisk",
  "Initializing on-device AI…": "Initialiserer AI på enheten …",
  "Initializing on-device AI...": "Initialiserer AI på enheten …",
  "Inlining images as base64": "Innebygde bilder som base64",
  "Input and output languages cannot be the same.":
    "Inndata- og utdataspråk kan ikke være de samme.",
  "Input language": "Inndataspråk",
  Instructions: "Instruksjoner",
  "Interface language": "Grensesnittspråk",
  Italian: "Italiensk",
  Japanese: "japansk",
  "Key Points": "Viktige punkter",
  "key-points": "nøkkelpunkter",
  Korean: "Koreansk",
  Length: "Lengde",
  Long: "Lang",
  long: "lang",
  Longer: "Lengre",
  longer: "lenger",
  Markdown: "Markdown",
  Maximize: "Maksimer",
  Medium: "Medium",
  medium: "medium",
  "More casual": "Mer avslappet",
  "More formal": "Mer formelt",
  "more-casual": "mer avslappet",
  "more-formal": "mer formell",
  Neutral: "Nøytral",
  neutral: "nøytral",
  No: "Nei",
  "No content generated.": "Ingen innhold generert.",
  "No issues found. Your text looks great!":
    "Ingen problemer funnet. Teksten din ser flott ut!",
  "No overrides": "Ingen overstyringer",
  Norwegian: "norsk",
  "Not sent": "Ikke sendt",
  "On-device model ready.": "Klar for modell på enheten.",
  "On-device model ready...": "On-device-modellen er klar...",
  Options: "Alternativer",
  "Output format": "Utdataformat",
  "Output language": "Utdataspråk",
  "Plain Text": "Ren tekst",
  Polish: "Pusse",
  Portuguese: "Portugisisk",
  "Powered by": "Drevet av",
  Preview: "Forhåndsvisning",
  Proofread: "Korrekturlesing",
  "Proofread again": "Korrekturles igjen",
  "Proofread on Backend": "Korrekturlesing på backend",
  Proofreading: "Korrekturlesing",
  "Proofreading part {current}/{total}...":
    "Korrekturleser del {current}/{total}...",
  "Ready.": "Klar.",
  "Received backend response.": "Mottok svar fra backend.",
  "Receiving response...": "Mottar svar...",
  Record: "Spill inn",
  "Record audio": "Ta opp lyd",
  "Recorded audio:": "Opptatt lyd:",
  "Recording...": "Tar opp...",
  Reference: "Referanse",
  References: "Referanser",
  Regenerate: "Regenerer",
  "Regenerate on Backend": "Regenerer på backend",
  "Reject response": "Avvis svar",
  "Remove image": "Fjern bilde",
  "Replace with": "Erstatt med",
  "Requesting signed URL": "Forespørsel om signert URL",
  "Response stopped.": "Svaret ble stoppet.",
  Reset: "Tilbakestill",
  "Reset conversation": "Tilbakestill samtalen",
  "Restore size": "Gjenopprett størrelse",
  Rewrite: "Omskrive",
  "Rewrite again": "Skriv om igjen",
  "Rewrite on Backend": "Omskriving på backend",
  "Rewriting part {current}/{total}...": "Omskriver del {current}/{total}...",
  "Rewriting text": "Omskriving av tekst",
  "Running a script…": "Kjører et skript…",
  "Running on-device...": "Kjører på enheten...",
  Russian: "Russisk",
  Search: "Søk",
  "Search the documentation…": "Søk i dokumentasjonen…",
  "Search with AI-Kit": "Søk med AI-Kit",
  "Searching knowledge base…": "Søker i kunnskapsbasen…",
  "Searching…": "Søker…",
  "Select or type tags…": "Velg eller skriv inn tagger…",
  "Select or type tags...": "Velg eller skriv inn tagger…",
  Send: "Send",
  "Sending request to backend…": "Sender forespørsel til backend…",
  "Sending request to server...": "Sender forespørsel til serveren...",
  Short: "Kort",
  short: "kort",
  Shorter: "Kortere",
  shorter: "kortere",
  Show: "Vise",
  "Sites searched: {count}": "Gjennomsøkte nettsteder: {count}",
  "Something went wrong.": "Noe gikk galt.",
  "Something went wrong. Please try again.": "Noe gikk galt. Prøv igjen.",
  "We couldn't verify that you're human. Please try again.": "Vi kunne ikke bekrefte at du er et menneske. Prøv igjen.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Du har ikke lov til å bruke denne AI-funksjonen. Logg på eller kontakt eieren av nettstedet.",
  "Too many requests. Please wait a moment and try again.": "For mange forespørsler. Vent et øyeblikk og prøv igjen.",
  "The request could not be processed. Review your input and try again.": "Forespørselen kunne ikke behandles. Kontroller inndataene og prøv igjen.",
  "We couldn't reach the AI service. Check your connection and try again.": "Vi kunne ikke nå AI-tjenesten. Sjekk forbindelsen og prøv igjen.",
  "The AI service is temporarily unavailable. Please try again.": "AI-tjenesten er midlertidig utilgjengelig. Prøv igjen.",
  "Request ID": "Forespørsels-ID",
  Sources: "Kilder",
  Spanish: "Spansk",
  Stop: "Stoppe",
  "Stop recording": "Stopp opptak",
  Subcategories: "Underkategorier",
  "Suggested change": "Foreslått endring",
  Summarize: "Oppsummer",
  "Summarize again": "Oppsummer igjen",
  "Summarize on Backend": "Oppsummer på backend",
  Summarizing: "Oppsummering",
  "Summarizing part {current}/{total}...":
    "Oppsummerer del {current}/{total}...",
  Swedish: "Svensk",
  Tags: "Tagger",
  Teaser: "Teaser",
  teaser: "teaser",
  Thai: "Thai",
  "The alt text for the image.": "Alt-teksten for bildet.",
  "The caption for the image.": "Bildeteksten til bildet.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "Den konfigurerte KI-modellen støtter ikke bildevedlegg. Kontakt nettstedsadministratoren.",
  "The description for the image.": "Beskrivelsen for bildet.",
  "The excerpt for the post.":
    "Valgfritt. Kort sammendrag som kan vises i søkeresultater og forhåndsvisninger.",
  "The format for the generated output.":
    "Formatet for det genererte resultatet.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "Språket AI-Kit skal bruke for generert tekst som standard (når det er aktuelt).",
  "The language of the input text.": "Språket i inndatateksten.",
  "The summary style to generate.": "Sammendragsstilen som skal genereres.",
  "The target output length.": "Målutgangslengden.",
  "The title for the image.": "Tittelen på bildet.",
  "The title for the post.": "Tittelen på innlegget.",
  "The tone or style for the AI to use.":
    "Tonen eller stilen som AI-en skal bruke.",
  "The topic or subject for the AI to write about.":
    "Emnet eller emnet AI-en skal skrive om.",
  "Thinking…": "Tenker…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Dette sammendraget ble generert av KI og kan inneholde feil. Kontroller viktig informasjon.",
  Title: "Tittel",
  "TL;DR": "TL;DR",
  tldr: "tldr",
  Tone: "Tone",
  Topic: "Emne",
  Translate: "Oversett",
  "Translate again": "Oversett på nytt",
  "Translate on Backend": "Oversett på backend",
  Translating: "Oversettelse",
  "Translating text": "Oversetter tekst",
  "Translating part {current}/{total}...":
    "Oversetter del {current}/{total}...",
  Turkish: "Tyrkisk",
  Type: "Type",
  Ukrainian: "Ukrainsk",
  "Unexpected error": "Uventet feil",
  Uploading: "Opplasting",
  "Uploading images via signed URL": "Laster opp bilder via signert URL",
  User: "Bruker",
  "View image": "Vis bilde",
  "Using backend": "Bruk av backend",
  "Using on-device": "Bruk på enheten",
  "Waiting for backend response…": "Venter på svar fra bakenden…",
  "Waiting for response...": "Venter på svar...",
  Why: "Hvorfor",
  "Working…": "Jobber…",
  "{name} is working…": "{name} arbeider…",
  "Generating response…": "Genererer svar…",
  "Knowledge Base searches: {count}": "Søk i kunnskapsbasen: {count}",
  "Websites searched: {count}": "Nettsteder undersøkt: {count}",
  "Other tools used: {count}": "Andre verktøy brukt: {count}",
  "Working...": "Jobber…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Skriv",
  "Writing part {current}/{total}...": "Skriver del {current}/{total}...",
  Yes: "Ja",
  "You need a subscription to use this feature on frontend.":
    "Du trenger et abonnement for å bruke denne funksjonen på frontend.",
};

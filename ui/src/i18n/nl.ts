export const nlDict: Record<string, string> = {
  "{name} is thinking…": "{name} denkt na…",
  "Sending request…": "Verzoek verzenden…",
  "Waiting for reply…": "Wachten op antwoord…",
  "Reconnecting…": "Opnieuw verbinden…",
  "Checking sources…": "Bronnen controleren…",
  "Knowledge base searched": "Kennisbank doorzocht",
  "Web search completed": "Webzoekopdracht voltooid",
  "Calculation completed": "Berekening voltooid",
  "Script completed": "Script voltooid",
  "Check completed": "Controle voltooid",
  "Action completed": "Actie voltooid",
  "Action failed": "Actie mislukt",
  "Search query: {query}": "Zoekopdracht: {query}",
  "Categories: {count}": "Categorieën: {count}",
  "Subcategories: {count}": "Subcategorieën: {count}",
  "Tags: {count}": "Tags: {count}",
  "Results: {count}": "Resultaten: {count}",
  "Retry": "Opnieuw proberen",
  "The answer could not be verified against its cited sources. Please try again.": "Het antwoord kon niet worden geverifieerd aan de hand van de aangehaalde bronnen. Probeer het opnieuw.",
  "The knowledge base search could not be completed. Please try again.": "De zoekopdracht in de kennisbank kon niet worden voltooid. Probeer het opnieuw.",
  "The answer's source check could not be completed. Please try again.": "De broncontrole van het antwoord kon niet worden voltooid. Probeer het opnieuw.",
  "Action interrupted": "Actie onderbroken",
  "Failed actions: {count}": "Mislukte acties: {count}",
  "--- Select ---": "--- Selecteren ---",
  "A response is still running. Wait for it to finish or cancel it.": "Er wordt nog een antwoord opgesteld. Wacht tot het klaar is of annuleer het.",
  Accept: "Accepteren",
  "Accept response": "Antwoord accepteren",
  Add: "Toevoegen",
  "Add image": "Afbeelding toevoegen",
  "Additional instructions to guide the AI.":
    "Aanvullende instructies om de AI te begeleiden.",
  "AI Assistant": "AI-assistent",
  "AI Summary": "AI-samenvatting",
  "Alt Text": "Alternatieve tekst",
  "An error occurred.": "Er is een fout opgetreden.",
  "An unknown error occurred.": "Er is een onbekende fout opgetreden.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Antwoorden van {name} worden gegenereerd door AI en kunnen fouten bevatten. Controleer belangrijke informatie.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Ik kon niet genoeg betrouwbare informatie vinden om dit te beantwoorden. Formuleer uw vraag anders of baken het onderwerp verder af.",
  "The AI response took too long. Please try again.":
    "Het AI-antwoord duurde te lang. Probeer het opnieuw.",
  Arabic: "Arabisch",
  "Are you sure you want to reset the conversation?":
    "Weet je zeker dat je het gesprek wilt resetten?",
  "As-Is": "Ongewijzigd",
  "as-is": "zoals het is",
  "Ask anything…": "Vraag maar raak…",
  "Ask me": "Vraag het mij",
  Assistant: "Assistent",
  "Assistant is thinking…": "De assistent denkt na…",
  "Audio message": "Audiobericht",
  "Audio no longer available": "Audio is niet langer beschikbaar",
  "Audio recorded": "Audio opgenomen",
  auto: "Automatisch",
  "Auto-detect": "Automatische detectie",
  "Calculating…": "Berekent…",
  Cancel: "Annuleren",
  Caption: "Ondertiteling",
  Casual: "Informeel",
  casual: "casual",
  Categories: "Categorieën",
  "Checking…": "Controleert…",
  "Checking capabilities…": "Mogelijkheden controleren…",
  "Checking capabilities...": "Mogelijkheden controleren…",
  Chinese: "Chinese",
  Clear: "Wissen",
  "Clear audio": "Audio wissen",
  "Click again to confirm": "Klik opnieuw om te bevestigen",
  Close: "Sluiten",
  "Close chat": "Chat sluiten",
  Confirm: "Bevestigen",
  Corrected: "Gecorrigeerd",
  "Creating final summary...": "Definitieve samenvatting maken...",
  "Creating LanguageDetector session": "Een LanguageDetector-sessie aanmaken",
  "Creating LanguageModel session": "Een LanguageModel-sessie aanmaken",
  "Creating Proofreader session": "Een proefleessessie aanmaken",
  "Creating Rewriter session": "Een herschrijfsessie aanmaken",
  "Creating Summarizer session": "Een samenvattingssessie aanmaken",
  "Creating Translator session": "Een vertaalsessie aanmaken",
  "Creating Writer session": "Een schrijfsessie aanmaken",
  "Deciding capability": "Beslissingsvermogen",
  Description: "Beschrijving",
  "Detecting language": "Taal detecteren",
  "Done.": "Klaar.",
  "Downloading model…": "Model downloaden…",
  "Downloading model...": "Model downloaden…",
  Dutch: "Nederlands",
  Edit: "Bewerken",
  "Empty response": "Leeg antwoord",
  English: "Engels",
  "Enter a search query to start.": "Voer een zoekopdracht in om te beginnen.",
  Error: "Fout",
  Excerpt: "Uittreksel",
  Filters: "Filters",
  Formal: "Formeel",
  formal: "formeel",
  French: "Frans",
  Generate: "Genereren",
  "Generate Image Metadata": "Afbeeldingsmetadata genereren",
  "Generate Post Metadata": "Berichtmetadata genereren",
  "Generated content": "Gegenereerde inhoud",
  "Generating text": "Tekst genereren",
  "Generating…": "Genereren…",
  "Generating...": "Genereren…",
  German: "Duits",
  Headline: "Kop",
  headline: "kop",
  Hebrew: "Hebreeuws",
  Hide: "Verbergen",
  Hindi: "Hindi",
  "Hover highlights to see explanations.":
    "Beweeg de muis over de gemarkeerde elementen om de uitleg te zien.",
  HTML: "HTML",
  Hungarian: "Hongaars",
  "I'm ready to assist you.": "Ik sta klaar om je te helpen.",
  "Image no longer available": "Afbeelding is niet langer beschikbaar",
  "Image preview": "Afbeeldingsvoorbeeld",
  Indonesian: "Indonesisch",
  "Initializing on-device AI…": "AI op het apparaat initialiseren…",
  "Initializing on-device AI...": "AI op het apparaat initialiseren…",
  "Inlining images as base64": "Afbeeldingen inline plaatsen als base64",
  "Input and output languages cannot be the same.":
    "De invoer- en uitvoertaal mogen niet dezelfde zijn.",
  "Input language": "Invoertaal",
  Instructions: "Instructies",
  "Interface language": "Interfacetaal",
  Italian: "Italiaans",
  Japanese: "Japanse",
  "Key Points": "Kernpunten",
  "key-points": "kernpunten",
  Korean: "Koreaans",
  Length: "Lengte",
  Long: "Lang",
  long: "lang",
  Longer: "Langer",
  longer: "langer",
  Markdown: "Markdown",
  Maximize: "Maximaliseren",
  Medium: "Medium",
  medium: "medium",
  "More casual": "Meer informeel",
  "More formal": "Meer formeel",
  "more-casual": "meer informeel",
  "more-formal": "formeler",
  Neutral: "Neutrale",
  neutral: "neutrale",
  No: "Nee",
  "No content generated.": "Geen inhoud gegenereerd.",
  "No issues found. Your text looks great!":
    "Geen problemen gevonden. Je tekst ziet er geweldig uit!",
  "No overrides": "Geen overrides",
  Norwegian: "Noors",
  "Not sent": "Niet verzonden",
  "On-device model ready.": "Model klaar voor gebruik op het apparaat.",
  "On-device model ready...": "On-device model gereed...",
  Options: "Opties",
  "Output format": "Uitvoerformaat",
  "Output language": "Uitvoertaal",
  "Plain Text": "Platte tekst",
  Polish: "Pools",
  Portuguese: "Portugees",
  "Powered by": "Mogelijk gemaakt door",
  Preview: "Voorbeeld",
  Proofread: "Proeflezen",
  "Proofread again": "Lees het nog eens na.",
  "Proofread on Backend": "Proeflezen in de backend",
  Proofreading: "Proeflezen",
  "Proofreading part {current}/{total}...":
    "Deel {current}/{total} proeflezen...",
  "Ready.": "Klaar.",
  "Received backend response.": "Reactie van de backend ontvangen.",
  "Receiving response...": "Antwoord ontvangen...",
  Record: "Opnemen",
  "Record audio": "Audio opnemen",
  "Recorded audio:": "Opgenomen audio:",
  "Recording...": "Bezig met opnemen...",
  Reference: "Referentie",
  References: "Referenties",
  Regenerate: "Regenereren",
  "Regenerate on Backend": "Genereer opnieuw op de backend",
  "Reject response": "Antwoord afwijzen",
  "Remove image": "Afbeelding verwijderen",
  "Replace with": "Vervangen door",
  "Requesting signed URL": "Verzoek om ondertekende URL",
  "Response stopped.": "Antwoord gestopt.",
  Reset: "Resetten",
  "Reset conversation": "Gesprek resetten",
  "Restore size": "Grootte herstellen",
  Rewrite: "Herschrijven",
  "Rewrite again": "Herschrijf het opnieuw",
  "Rewrite on Backend": "Herschrijven in de backend",
  "Rewriting part {current}/{total}...":
    "Deel {current}/{total} herschrijven...",
  "Rewriting text": "Tekst herschrijven",
  "Running a script…": "Voert een script uit…",
  "Running on-device...": "Wordt on-device uitgevoerd...",
  Russian: "Russisch",
  Search: "Zoekopdracht",
  "Search the documentation…": "Doorzoek de documentatie…",
  "Search with AI-Kit": "Zoeken met AI-Kit",
  "Searching knowledge base…": "Zoekt in de kennisbank…",
  "Searching…": "Zoeken…",
  "Select or type tags…": "Selecteer of typ tags…",
  "Select or type tags...": "Selecteer of typ tags…",
  Send: "Versturen",
  "Sending request to backend…": "Verzoek naar de backend verzenden…",
  "Sending request to server...": "Verzoek naar server verzenden...",
  Short: "Kort",
  short: "kort",
  Shorter: "Korter",
  shorter: "korter",
  Show: "Show",
  "Sites searched: {count}": "Doorzochte websites: {count}",
  "Something went wrong.": "Er is iets misgegaan.",
  "Something went wrong. Please try again.": "Er is iets misgegaan. Probeer het opnieuw.",
  "We couldn't verify that you're human. Please try again.": "We konden niet verifiëren dat je een mens bent. Probeer het opnieuw.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Je bent niet gemachtigd om deze AI-functie te gebruiken. Meld je aan of neem contact op met de eigenaar van de site.",
  "Too many requests. Please wait a moment and try again.": "Te veel aanvragen. Wacht even en probeer het opnieuw.",
  "The request could not be processed. Review your input and try again.": "De aanvraag kon niet worden verwerkt. Controleer je invoer en probeer het opnieuw.",
  "We couldn't reach the AI service. Check your connection and try again.": "We konden de AI-service niet bereiken. Controleer je verbinding en probeer het opnieuw.",
  "The AI service is temporarily unavailable. Please try again.": "De AI-service is tijdelijk niet beschikbaar. Probeer het opnieuw.",
  "Request ID": "Verzoek-ID",
  Sources: "Bronnen",
  Spanish: "Spaans",
  Stop: "Stop",
  "Stop recording": "Opname stoppen",
  Subcategories: "Subcategorieën",
  "Suggested change": "Voorgestelde wijziging",
  Summarize: "Samenvatten",
  "Summarize again": "Vat het nogmaals samen.",
  "Summarize on Backend": "Samenvatten in de backend",
  Summarizing: "Samenvattend",
  "Summarizing part {current}/{total}...":
    "Deel {current}/{total} samenvatten...",
  Swedish: "Zweeds",
  Tags: "Tags",
  Teaser: "Teaser",
  teaser: "teaser",
  Thai: "Thais",
  "The alt text for the image.": "De alternatieve tekst voor de afbeelding.",
  "The caption for the image.": "Het onderschrift bij de afbeelding.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "Het geconfigureerde AI-model ondersteunt geen afbeeldingsbijlagen. Neem contact op met de sitebeheerder.",
  "The description for the image.": "De beschrijving bij de afbeelding.",
  "The excerpt for the post.":
    "Optioneel. Korte samenvatting die kan verschijnen in zoekresultaten en previews.",
  "The format for the generated output.":
    "Het formaat voor de gegenereerde output.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "De taal die AI-Kit standaard moet gebruiken voor gegenereerde tekst (indien van toepassing).",
  "The language of the input text.": "De taal van de invoertekst.",
  "The summary style to generate.": "De te genereren samenvattingsstijl.",
  "The target output length.": "De gewenste lengte van de uitvoer.",
  "The title for the image.": "De titel voor de afbeelding.",
  "The title for the post.": "De titel voor het bericht.",
  "The tone or style for the AI to use.":
    "De toon of stijl die de AI moet gebruiken.",
  "The topic or subject for the AI to write about.":
    "Het onderwerp waarover de AI moet schrijven.",
  "Thinking…": "Denkt na…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Deze samenvatting is gegenereerd door AI en kan fouten bevatten. Controleer belangrijke informatie.",
  Title: "Titel",
  "TL;DR": "Kort samengevat:",
  tldr: "tldr",
  Tone: "Toon",
  Topic: "Onderwerp",
  Translate: "Vertalen",
  "Translate again": "Vertaal opnieuw",
  "Translate on Backend": "Vertalen in de backend",
  Translating: "Vertalen",
  "Translating text": "Tekst vertalen",
  "Translating part {current}/{total}...": "Deel {current}/{total} vertalen...",
  Turkish: "Turks",
  Type: "Type",
  Ukrainian: "Oekraïens",
  "Unexpected error": "Onverwachte fout",
  Uploading: "Uploaden",
  "Uploading images via signed URL":
    "Afbeeldingen uploaden via een ondertekende URL",
  User: "Gebruiker",
  "View image": "Afbeelding bekijken",
  "Using backend": "Gebruikmaken van de backend",
  "Using on-device": "Gebruik op het apparaat",
  "Waiting for backend response…": "Wachten op reactie van de backend…",
  "Waiting for response...": "Wachten op antwoord...",
  Why: "Waarom",
  "Working…": "Bezig…",
  "{name} is working…": "{name} is aan het werk…",
  "Generating response…": "Antwoord genereren…",
  "Knowledge Base searches: {count}": "Zoekopdrachten in de kennisbank: {count}",
  "Websites searched: {count}": "Doorzochte websites: {count}",
  "Other tools used: {count}": "Andere gebruikte hulpmiddelen: {count}",
  "Working...": "Bezig…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Schrijven",
  "Writing part {current}/{total}...": "Deel {current}/{total} schrijven...",
  Yes: "Ja",
  "You need a subscription to use this feature on frontend.":
    "Je hebt een abonnement nodig om deze functie op de frontend te gebruiken.",
};

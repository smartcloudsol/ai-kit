export const idDict: Record<string, string> = {
  "{name} is thinking…": "{name} sedang berpikir…",
  "Sending request…": "Mengirim permintaan…",
  "Waiting for reply…": "Menunggu jawaban…",
  "Reconnecting…": "Menghubungkan kembali…",
  "Checking sources…": "Memeriksa sumber…",
  "Knowledge base searched": "Pencarian basis pengetahuan selesai",
  "Web search completed": "Pencarian web selesai",
  "Calculation completed": "Perhitungan selesai",
  "Script completed": "Skrip selesai",
  "Check completed": "Pemeriksaan selesai",
  "Action completed": "Tindakan selesai",
  "Action failed": "Tindakan gagal",
  "Search query: {query}": "Kueri pencarian: {query}",
  "Categories: {count}": "Kategori: {count}",
  "Subcategories: {count}": "Subkategori: {count}",
  "Tags: {count}": "Tag: {count}",
  "Results: {count}": "Hasil: {count}",
  "Retry": "Coba lagi",
  "The answer could not be verified against its cited sources. Please try again.": "Jawaban tidak dapat diverifikasi berdasarkan sumber yang dikutip. Silakan coba lagi.",
  "The knowledge base search could not be completed. Please try again.": "Pencarian basis pengetahuan tidak dapat diselesaikan. Silakan coba lagi.",
  "The answer's source check could not be completed. Please try again.": "Pemeriksaan sumber jawaban tidak dapat diselesaikan. Silakan coba lagi.",
  "Action interrupted": "Tindakan terhenti",
  "Failed actions: {count}": "Tindakan gagal: {count}",
  "--- Select ---": "--- Pilih ---",
  "A response is still running. Wait for it to finish or cancel it.": "Jawaban masih sedang dibuat. Tunggu hingga selesai atau batalkan.",
  Accept: "Terima",
  "Accept response": "Terima jawaban",
  Add: "Tambah",
  "Add image": "Tambah gambar",
  "Additional instructions to guide the AI.":
    "Instruksi tambahan untuk memandu AI.",
  "AI Assistant": "Asisten AI",
  "AI Summary": "Ringkasan AI",
  "Alt Text": "Teks Alternatif",
  "An error occurred.": "Terjadi kesalahan.",
  "An unknown error occurred.": "Terjadi kesalahan yang tidak diketahui.",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "Jawaban dari {name} dibuat oleh AI dan mungkin mengandung kesalahan. Periksa informasi penting.",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "Saya tidak menemukan cukup informasi tepercaya untuk menjawabnya. Silakan rumuskan ulang pertanyaan atau persempit topiknya.",
  "The AI response took too long. Please try again.":
    "Respons AI memerlukan waktu terlalu lama. Silakan coba lagi.",
  Arabic: "Arab",
  "Are you sure you want to reset the conversation?":
    "Yakin ingin mereset percakapan?",
  "As-Is": "Tanpa perubahan",
  "as-is": "dengan adanya",
  "Ask anything…": "Tanyakan apa saja…",
  "Ask me": "Tanya saya",
  Assistant: "Asisten",
  "Assistant is thinking…": "Asisten sedang berpikir…",
  "Audio message": "Pesan audio",
  "Audio no longer available": "Audio tidak lagi tersedia",
  "Audio recorded": "Audio direkam",
  auto: "mobil",
  "Auto-detect": "Deteksi otomatis",
  "Calculating…": "Sedang menghitung…",
  Cancel: "Membatalkan",
  Caption: "Keterangan",
  Casual: "Kasual",
  casual: "kasual",
  Categories: "Kategori",
  "Checking…": "Sedang memeriksa…",
  "Checking capabilities…": "Memeriksa kemampuan…",
  "Checking capabilities...": "Memeriksa kemampuan…",
  Chinese: "Cina",
  Clear: "Hapus",
  "Clear audio": "Hapus audio",
  "Click again to confirm": "Klik lagi untuk mengonfirmasi",
  Close: "Menutup",
  "Close chat": "Tutup chat",
  Confirm: "Konfirmasi",
  Corrected: "Dikoreksi",
  "Creating final summary...": "Membuat ringkasan akhir...",
  "Creating LanguageDetector session": "Membuat sesi LanguageDetector",
  "Creating LanguageModel session": "Membuat sesi LanguageModel",
  "Creating Proofreader session": "Membuat sesi pemeriksa ejaan",
  "Creating Rewriter session": "Membuat sesi Penulis Ulang",
  "Creating Summarizer session": "Membuat sesi Summarizer",
  "Creating Translator session": "Membuat sesi Penerjemah",
  "Creating Writer session": "Membuat sesi Penulis",
  "Deciding capability": "Kemampuan pengambilan keputusan",
  Description: "Keterangan",
  "Detecting language": "Mendeteksi bahasa",
  "Done.": "Selesai.",
  "Downloading model…": "Mengunduh model…",
  "Downloading model...": "Mengunduh model…",
  Dutch: "Belanda",
  Edit: "Edit",
  "Empty response": "Jawaban kosong",
  English: "Bahasa inggris",
  "Enter a search query to start.":
    "Masukkan kata kunci pencarian untuk memulai.",
  Error: "Kesalahan",
  Excerpt: "Kutipan",
  Filters: "Filter",
  Formal: "Resmi",
  formal: "resmi",
  French: "Perancis",
  Generate: "Buat",
  "Generate Image Metadata": "Buat metadata gambar",
  "Generate Post Metadata": "Buat metadata pos",
  "Generated content": "Konten yang dihasilkan",
  "Generating text": "Menghasilkan teks",
  "Generating…": "Sedang menghasilkan…",
  "Generating...": "Sedang menghasilkan…",
  German: "Jerman",
  Headline: "Judul berita",
  headline: "judul",
  Hebrew: "Ibrani",
  Hide: "Bersembunyi",
  Hindi: "Hindi",
  "Hover highlights to see explanations.":
    "Arahkan kursor ke item yang disorot untuk melihat penjelasannya.",
  HTML: "HTML",
  Hungarian: "Hongaria",
  "I'm ready to assist you.": "Saya siap membantu Anda.",
  "Image no longer available": "Gambar tidak lagi tersedia",
  "Image preview": "Pratinjau gambar",
  Indonesian: "Indonesia",
  "Initializing on-device AI…": "Menginisialisasi AI pada perangkat…",
  "Initializing on-device AI...": "Menginisialisasi AI pada perangkat…",
  "Inlining images as base64": "Menyisipkan gambar sebagai base64",
  "Input and output languages cannot be the same.":
    "Bahasa masukan dan bahasa keluaran tidak boleh sama.",
  "Input language": "Bahasa masukan",
  Instructions: "Instruksi",
  "Interface language": "Bahasa antarmuka",
  Italian: "Italia",
  Japanese: "Jepang",
  "Key Points": "Poin-Poin Penting",
  "key-points": "poin-poin penting",
  Korean: "Korea",
  Length: "Panjang",
  Long: "Panjang",
  long: "panjang",
  Longer: "Lebih lama",
  longer: "lebih lama",
  Markdown: "Markdown",
  Maximize: "Maksimalkan",
  Medium: "Sedang",
  medium: "sedang",
  "More casual": "Lebih kasual",
  "More formal": "Lebih formal",
  "more-casual": "lebih kasual",
  "more-formal": "lebih formal",
  Neutral: "Netral",
  neutral: "netral",
  No: "Tidak",
  "No content generated.": "Tidak ada konten yang dihasilkan.",
  "No issues found. Your text looks great!":
    "Tidak ditemukan masalah. Teks Anda terlihat bagus!",
  "No overrides": "Tidak ada penggantian",
  Norwegian: "Norwegia",
  "Not sent": "Tidak terkirim",
  "On-device model ready.": "Model siap pakai di perangkat.",
  "On-device model ready...": "Model di perangkat siap...",
  Options: "Opsi",
  "Output format": "Format keluaran",
  "Output language": "Bahasa keluaran",
  "Plain Text": "Teks biasa",
  Polish: "Polandia",
  Portuguese: "Portugis",
  "Powered by": "Didukung oleh",
  Preview: "Pratinjau",
  Proofread: "Periksa",
  "Proofread again": "Periksa kembali",
  "Proofread on Backend": "Pemeriksaan tata bahasa di bagian backend.",
  Proofreading: "Pemeriksaan tata bahasa",
  "Proofreading part {current}/{total}...":
    "Memeriksa bagian {current}/{total}...",
  "Ready.": "Siap.",
  "Received backend response.": "Respons dari backend telah diterima.",
  "Receiving response...": "Menerima respons...",
  Record: "Rekam",
  "Record audio": "Rekam audio",
  "Recorded audio:": "Audio direkam:",
  "Recording...": "Merekam...",
  Reference: "Referensi",
  References: "Referensi",
  Regenerate: "Buat ulang",
  "Regenerate on Backend": "Lakukan regenerasi di backend.",
  "Reject response": "Tolak jawaban",
  "Remove image": "Hapus gambar",
  "Replace with": "Ganti dengan",
  "Requesting signed URL": "Meminta URL yang ditandatangani",
  "Response stopped.": "Respons dihentikan.",
  Reset: "Atur ulang",
  "Reset conversation": "Reset percakapan",
  "Restore size": "Kembalikan ukuran",
  Rewrite: "Menulis kembali",
  "Rewrite again": "Tulis ulang lagi",
  "Rewrite on Backend": "Penulisan ulang pada bagian Backend",
  "Rewriting part {current}/{total}...":
    "Menulis ulang bagian {current}/{total}...",
  "Rewriting text": "Menulis ulang teks",
  "Running a script…": "Sedang menjalankan skrip…",
  "Running on-device...": "Berjalan di perangkat...",
  Russian: "Rusia",
  Search: "Mencari",
  "Search the documentation…": "Cari di dokumentasi…",
  "Search with AI-Kit": "Cari dengan AI-Kit",
  "Searching knowledge base…": "Sedang mencari di basis pengetahuan…",
  "Searching…": "Sedang mencari…",
  "Select or type tags…": "Pilih atau ketik tag…",
  "Select or type tags...": "Pilih atau ketik tag…",
  Send: "Kirim",
  "Sending request to backend…": "Mengirim permintaan ke backend…",
  "Sending request to server...": "Mengirim permintaan ke server...",
  Short: "Pendek",
  short: "pendek",
  Shorter: "Singkat",
  shorter: "singkat",
  Show: "Menunjukkan",
  "Sites searched: {count}": "Situs yang ditelusuri: {count}",
  "Something went wrong.": "Terjadi kesalahan.",
  "Something went wrong. Please try again.": "Terjadi kesalahan. Silakan coba lagi.",
  "We couldn't verify that you're human. Please try again.": "Kami tidak dapat memverifikasi bahwa Anda manusia. Silakan coba lagi.",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "Anda tidak berwenang menggunakan fitur AI ini. Silakan masuk atau hubungi pemilik situs.",
  "Too many requests. Please wait a moment and try again.": "Terlalu banyak permintaan. Mohon tunggu sesaat dan coba lagi.",
  "The request could not be processed. Review your input and try again.": "Permintaan tidak dapat diproses. Tinjau masukan Anda dan coba lagi.",
  "We couldn't reach the AI service. Check your connection and try again.": "Kami tidak dapat menjangkau layanan AI. Periksa koneksi Anda dan coba lagi.",
  "The AI service is temporarily unavailable. Please try again.": "Layanan AI tidak tersedia sementara. Silakan coba lagi.",
  "Request ID": "ID Permintaan",
  Sources: "Sumber",
  Spanish: "Spanyol",
  Stop: "Berhenti",
  "Stop recording": "Hentikan perekaman",
  Subcategories: "Subkategori",
  "Suggested change": "Perubahan yang disarankan",
  Summarize: "Meringkaskan",
  "Summarize again": "Ringkas kembali",
  "Summarize on Backend": "Ringkaskan bagian Backend",
  Summarizing: "Ringkasan",
  "Summarizing part {current}/{total}...":
    "Merangkum bagian {current}/{total}...",
  Swedish: "Swedia",
  Tags: "Tag",
  Teaser: "Teks teaser",
  teaser: "pengejek",
  Thai: "Thai",
  "The alt text for the image.": "Teks alt untuk gambar.",
  "The caption for the image.": "Keterangan untuk gambar tersebut.",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "Model AI yang dikonfigurasi tidak mendukung lampiran gambar. Hubungi administrator situs.",
  "The description for the image.": "Deskripsi untuk gambar tersebut.",
  "The excerpt for the post.":
    "Opsional. Ringkasan singkat yang dapat muncul di hasil pencarian dan pratinjau.",
  "The format for the generated output.": "Format output yang dihasilkan.",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "Bahasa yang seharusnya digunakan AI-Kit untuk teks yang dihasilkan secara default (jika berlaku).",
  "The language of the input text.": "Bahasa dari teks masukan.",
  "The summary style to generate.": "Gaya ringkasan yang akan dihasilkan.",
  "The target output length.": "Panjang keluaran target.",
  "The title for the image.": "Judul untuk gambar tersebut.",
  "The title for the post.": "Judul untuk postingan tersebut.",
  "The tone or style for the AI to use.":
    "Nada atau gaya yang akan digunakan oleh AI.",
  "The topic or subject for the AI to write about.":
    "Topik atau subjek yang akan ditulis oleh AI.",
  "Thinking…": "Sedang berpikir…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "Ringkasan ini dibuat oleh AI dan mungkin mengandung kesalahan. Periksa informasi penting.",
  Title: "Judul",
  "TL;DR": "Singkatnya",
  tldr: "tldr",
  Tone: "Nada",
  Topic: "Topik",
  Translate: "Terjemahkan",
  "Translate again": "Terjemahkan lagi",
  "Translate on Backend": "Terjemahkan di Backend",
  Translating: "Menerjemahkan",
  "Translating text": "Menerjemahkan teks",
  "Translating part {current}/{total}...":
    "Menerjemahkan bagian {current}/{total}...",
  Turkish: "Turki",
  Type: "Jenis",
  Ukrainian: "Ukraina",
  "Unexpected error": "Kesalahan tak terduga",
  Uploading: "Mengunggah",
  "Uploading images via signed URL":
    "Mengunggah gambar melalui URL yang ditandatangani",
  User: "Pengguna",
  "View image": "Lihat gambar",
  "Using backend": "Menggunakan backend",
  "Using on-device": "Menggunakan perangkat",
  "Waiting for backend response…": "Menunggu respons dari backend…",
  "Waiting for response...": "Menunggu respons...",
  Why: "Mengapa",
  "Working…": "Memproses…",
  "{name} is working…": "{name} sedang bekerja…",
  "Generating response…": "Sedang menyusun jawaban…",
  "Knowledge Base searches: {count}": "Pencarian basis pengetahuan: {count}",
  "Websites searched: {count}": "Situs web yang ditelusuri: {count}",
  "Other tools used: {count}": "Alat lain yang digunakan: {count}",
  "Working...": "Memproses…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "Tulis",
  "Writing part {current}/{total}...": "Menulis bagian {current}/{total}...",
  Yes: "Ya",
  "You need a subscription to use this feature on frontend.":
    "Anda memerlukan langganan untuk menggunakan fitur ini di bagian depan (frontend).",
};

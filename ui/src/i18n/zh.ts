export const zhDict: Record<string, string> = {
  "{name} is thinking…": "{name}正在思考…",
  "Sending request…": "正在发送请求…",
  "Waiting for reply…": "正在等待回复…",
  "Reconnecting…": "正在重新连接…",
  "Checking sources…": "正在核查来源…",
  "Knowledge base searched": "知识库搜索已完成",
  "Web search completed": "网页搜索已完成",
  "Calculation completed": "计算已完成",
  "Script completed": "脚本执行已完成",
  "Check completed": "检查已完成",
  "Action completed": "操作已完成",
  "Action failed": "操作失败",
  "Search query: {query}": "搜索词：{query}",
  "Categories: {count}": "分类：{count}",
  "Subcategories: {count}": "子分类：{count}",
  "Tags: {count}": "标签：{count}",
  "Results: {count}": "结果：{count}",
  "Retry": "重试",
  "The answer could not be verified against its cited sources. Please try again.": "无法根据引用的来源验证答案。请重试。",
  "The knowledge base search could not be completed. Please try again.": "未能完成知识库搜索。请重试。",
  "The answer's source check could not be completed. Please try again.": "未能完成答案的来源核查。请重试。",
  "Action interrupted": "操作已中断",
  "Failed actions: {count}": "失败的操作：{count}",
  "--- Select ---": "--- 选择 ---",
  "A response is still running. Wait for it to finish or cancel it.": "仍在生成回答。请等待完成或将其取消。",
  Accept: "接受",
  "Accept response": "接受回复",
  Add: "添加",
  "Add image": "添加图片",
  "Additional instructions to guide the AI.": "指导人工智能的附加指令。",
  "AI Assistant": "AI 助手",
  "AI Summary": "人工智能概要",
  "Alt Text": "替代文字",
  "An error occurred.": "发生错误。",
  "An unknown error occurred.": "发生未知错误。",
  "Answers from {name} are generated by AI and may contain mistakes. Check important information.":
    "{name} 的回答由 AI 生成，可能包含错误。请核实重要信息。",
  "I couldn't find enough reliable information to answer that. Please rephrase the question or narrow the topic.":
    "我找不到足够可靠的信息来回答。请换一种方式提问，或缩小主题范围。",
  "The AI response took too long. Please try again.":
    "AI 响应耗时过长。请重试。",
  Arabic: "阿拉伯",
  "Are you sure you want to reset the conversation?": "确定要重置对话吗？",
  "As-Is": "不变",
  "as-is": "原样",
  "Ask anything…": "随便问…",
  "Ask me": "问我",
  Assistant: "助手",
  "Assistant is thinking…": "助手正在思考……",
  "Audio message": "音频消息",
  "Audio no longer available": "音频已不可用",
  "Audio recorded": "音频已录制",
  auto: "汽车",
  "Auto-detect": "自动检测",
  "Calculating…": "正在计算…",
  Cancel: "取消",
  Caption: "标题",
  Casual: "随意的",
  casual: "随意的",
  Categories: "分类",
  "Checking…": "正在检查…",
  "Checking capabilities…": "正在检查功能……",
  "Checking capabilities...": "正在检查功能……",
  Chinese: "中国人",
  Clear: "清除",
  "Clear audio": "清除音频",
  "Click again to confirm": "再次点击以确认",
  Close: "关闭",
  "Close chat": "关闭聊天",
  Confirm: "确认",
  Corrected: "已更正",
  "Creating final summary...": "正在生成最终摘要...",
  "Creating LanguageDetector session": "创建 LanguageDetector 会话",
  "Creating LanguageModel session": "创建 LanguageModel 会话",
  "Creating Proofreader session": "创建校对会议",
  "Creating Rewriter session": "创建 Rewriter 会话",
  "Creating Summarizer session": "创建摘要器会话",
  "Creating Translator session": "创建翻译会话",
  "Creating Writer session": "创建作家会议",
  "Deciding capability": "决策能力",
  Description: "描述",
  "Detecting language": "语言检测",
  "Done.": "完毕。",
  "Downloading model…": "正在下载模型…",
  "Downloading model...": "正在下载模型…",
  Dutch: "荷兰语",
  Edit: "编辑",
  "Empty response": "空回复",
  English: "英语",
  "Enter a search query to start.": "输入搜索词开始搜索。",
  Error: "错误",
  Excerpt: "摘抄",
  Filters: "筛选条件",
  Formal: "正式的",
  formal: "正式的",
  French: "法语",
  Generate: "生成",
  "Generate Image Metadata": "生成图片元数据",
  "Generate Post Metadata": "生成文章元数据",
  "Generated content": "生成的内容",
  "Generating text": "生成文本",
  "Generating…": "正在生成…",
  "Generating...": "正在生成…",
  German: "德语",
  Headline: "标题",
  headline: "标题",
  Hebrew: "希伯来语",
  Hide: "隐藏",
  Hindi: "印地语",
  "Hover highlights to see explanations.": "将鼠标悬停在高亮部分即可查看说明。",
  HTML: "HTML",
  Hungarian: "匈牙利",
  "I'm ready to assist you.": "我已准备好为你提供帮助。",
  "Image no longer available": "图片已不可用",
  "Image preview": "图片预览",
  Indonesian: "印度尼西亚",
  "Initializing on-device AI…": "正在初始化设备端人工智能……",
  "Initializing on-device AI...": "正在初始化设备端人工智能……",
  "Inlining images as base64": "将图像内联为 base64 格式",
  "Input and output languages cannot be the same.":
    "输入语言和输出语言不能相同。",
  "Input language": "输入语言",
  Instructions: "指示",
  "Interface language": "界面语言",
  Italian: "意大利语",
  Japanese: "日本人",
  "Key Points": "要点",
  "key-points": "要点",
  Korean: "韩国人",
  Length: "长度",
  Long: "长的",
  long: "长的",
  Longer: "更长",
  longer: "更长",
  Markdown: "Markdown",
  Maximize: "最大化",
  Medium: "中等的",
  medium: "中等的",
  "More casual": "更休闲",
  "More formal": "更正式",
  "more-casual": "更随意",
  "more-formal": "更正式的",
  Neutral: "中性的",
  neutral: "中性的",
  No: "否",
  "No content generated.": "未生成内容。",
  "No issues found. Your text looks great!": "未发现问题。您的文本看起来很棒！",
  "No overrides": "不覆盖",
  Norwegian: "挪威",
  "Not sent": "未发送",
  "On-device model ready.": "设备端模型已准备就绪。",
  "On-device model ready...": "端侧模型已就绪...",
  Options: "选项",
  "Output format": "输出格式",
  "Output language": "输出语言",
  "Plain Text": "纯文本",
  Polish: "抛光",
  Portuguese: "葡萄牙语",
  "Powered by": "供电",
  Preview: "预览",
  Proofread: "校对",
  "Proofread again": "再次校对",
  "Proofread on Backend": "后端校对",
  Proofreading: "校对中",
  "Proofreading part {current}/{total}...":
    "正在校对第 {current}/{total} 部分...",
  "Ready.": "就绪。",
  "Received backend response.": "已收到后端响应。",
  "Receiving response...": "正在接收响应...",
  Record: "录音",
  "Record audio": "录制音频",
  "Recorded audio:": "已录制音频:",
  "Recording...": "录制中...",
  Reference: "参考",
  References: "参考资料",
  Regenerate: "重新生成",
  "Regenerate on Backend": "后端重新生成",
  "Reject response": "拒绝回复",
  "Remove image": "移除图片",
  "Replace with": "替换为",
  "Requesting signed URL": "请求已签名 URL",
  "Response stopped.": "已停止回答。",
  Reset: "重置",
  "Reset conversation": "重置对话",
  "Restore size": "恢复大小",
  Rewrite: "改写",
  "Rewrite again": "重写",
  "Rewrite on Backend": "后端重写",
  "Rewriting part {current}/{total}...": "正在重写第 {current}/{total} 部分...",
  "Rewriting text": "重写文本",
  "Running a script…": "正在运行脚本…",
  "Running on-device...": "正在端侧运行...",
  Russian: "俄语",
  Search: "搜索",
  "Search the documentation…": "查找文档……",
  "Search with AI-Kit": "使用 AI 工具包进行搜索",
  "Searching knowledge base…": "正在搜索知识库…",
  "Searching…": "正在搜索……",
  "Select or type tags…": "选择或输入标签…",
  "Select or type tags...": "选择或输入标签…",
  Send: "发送",
  "Sending request to backend…": "正在向后端发送请求…",
  "Sending request to server...": "正在向服务器发送请求...",
  Short: "短的",
  short: "短的",
  Shorter: "较短",
  shorter: "较短",
  Show: "展示",
  "Sites searched: {count}": "已搜索的网站：{count} 个",
  "Something went wrong.": "出问题了。",
  "Something went wrong. Please try again.": "出了点问题，请重试。",
  "We couldn't verify that you're human. Please try again.": "无法验证您是人类，请重试。",
  "You are not authorized to use this AI feature. Please sign in or contact the site owner.": "您无权使用此 AI 功能，请登录或联系网站所有者。",
  "Too many requests. Please wait a moment and try again.": "请求过于频繁，请稍等片刻后重试。",
  "The request could not be processed. Review your input and try again.": "请求无法处理，请检查你的输入并重试。",
  "We couldn't reach the AI service. Check your connection and try again.": "无法连接到 AI 服务，请检查你的网络连接并重试。",
  "The AI service is temporarily unavailable. Please try again.": "AI 服务暂时不可用，请稍后再试。",
  "Request ID": "请求 ID",
  Sources: "来源",
  Spanish: "西班牙语",
  Stop: "停止",
  "Stop recording": "停止录制",
  Subcategories: "子分类",
  "Suggested change": "建议的更改",
  Summarize: "总结",
  "Summarize again": "再次总结",
  "Summarize on Backend": "后端总结",
  Summarizing: "总结",
  "Summarizing part {current}/{total}...":
    "正在总结第 {current}/{total} 部分...",
  Swedish: "瑞典",
  Tags: "标签",
  Teaser: "引导文案",
  teaser: "预告片",
  Thai: "泰国",
  "The alt text for the image.": "图片的替代文字。",
  "The caption for the image.": "图片说明文字。",
  "The configured AI model does not support image attachments. Contact the site administrator.":
    "配置的 AI 模型不支持图片附件。请联系网站管理员。",
  "The description for the image.": "图片描述。",
  "The excerpt for the post.": "可选。可能显示在搜索结果和预览中的简短摘要。",
  "The format for the generated output.": "生成输出的格式。",
  "The language AI-Kit should use for generated text by default (when applicable).":
    "AIKit 默认用于生成文本的语言（如适用）。",
  "The language of the input text.": "输入文本的语言。",
  "The summary style to generate.": "要生成的摘要样式。",
  "The target output length.": "目标输出长度。",
  "The title for the image.": "图片标题。",
  "The title for the post.": "帖子标题。",
  "The tone or style for the AI to use.": "人工智能使用的语气或风格。",
  "The topic or subject for the AI to write about.":
    "人工智能要撰写的主题或题目。",
  "Thinking…": "正在思考…",
  "This summary was generated by AI and may contain mistakes. Check important information.":
    "此摘要由 AI 生成，可能包含错误。请核实重要信息。",
  Title: "标题",
  "TL;DR": "简述",
  tldr: "太长不看",
  Tone: "语气",
  Topic: "话题",
  Translate: "翻译",
  "Translate again": "再次翻译",
  "Translate on Backend": "后端翻译",
  Translating: "翻译",
  "Translating text": "正在翻译文本",
  "Translating part {current}/{total}...":
    "正在翻译第 {current}/{total} 部分...",
  Turkish: "土耳其",
  Type: "类型",
  Ukrainian: "乌克兰",
  "Unexpected error": "意外错误",
  Uploading: "上传",
  "Uploading images via signed URL": "通过已签名 URL 上传图片",
  User: "用户",
  "View image": "查看图片",
  "Using backend": "使用后端",
  "Using on-device": "使用设备上的设备",
  "Waiting for backend response…": "正在等待后端响应……",
  "Waiting for response...": "正在等待响应...",
  Why: "为什么",
  "Working…": "正在处理…",
  "{name} is working…": "{name} 正在处理…",
  "Generating response…": "正在生成回复…",
  "Knowledge Base searches: {count}": "知识库搜索次数：{count}",
  "Websites searched: {count}": "已搜索网站：{count}",
  "Other tools used: {count}": "其他工具使用次数：{count}",
  "Working...": "正在处理…",
  "WPSuite AI-Kit": "WPSuite AI-Kit",
  Write: "撰写",
  "Writing part {current}/{total}...": "正在撰写第 {current}/{total} 部分...",
  Yes: "是",
  "You need a subscription to use this feature on frontend.":
    "您需要订阅才能在前端使用此功能。",
};
